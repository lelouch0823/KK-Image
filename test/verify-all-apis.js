...
async function runTests() {
  ...
  const permCheckRes = await request('POST', '/api/v1/permissions/check', {
  ...
  // V1 Update Folder
  const v1UpdateFolderRes = await request('PUT', `/api/v1/folders/${v1FolderId}`, {
    name: 'V1 Updated Folder',
  });
  assertStatus(v1UpdateFolderRes, 200, 'V1 Update Folder failed');
  logSuccess('PUT /api/v1/folders/:id');

  // V1 Share Settings
  const v1ShareRes = await request('PUT', `/api/v1/folders/${v1FolderId}/share`, {
    isPublic: true,
  });
  ...
  const updateFolderRes = await request('PUT', `/api/manage/folders/${testFolderId}`, {
    name: 'Updated Folder',
  });
  ...
  // V1 Update File
  const updateFileRes = await request('PUT', `/api/v1/files/${testFileId}`, {
    name: 'renamed.txt',
  });
  ...
  // Manage Rename File
  const manageRenameRes = await request('PUT', `/api/manage/files/${testFileId}`, {
    name: 'final-name.txt',
  });
  ...
  const file2Res = await request('POST', '/api/v1/files', {
    name: 'batch-test.txt',
    folderId: testFolderId,
  });
  ...
  // Create Album
  const createAlbumRes = await request('POST', '/api/manage/albums', {
    name: 'Test Album',
    isPublic: true,
  });
  ...
  // Add File to Album
  const addFileAlbumRes = await request('POST', `/api/manage/albums/${albumId}/files`, {
    fileIds: [testFileId],
  });
  ...
  // Update Album
  const updateAlbumRes = await request('PUT', `/api/manage/albums/${albumId}`, {
    name: 'Updated Album',
  });
  ...
  const removeFileAlbumRes = await request('DELETE', `/api/manage/albums/${albumId}/files`, {
    fileIds: [testFileId],
  });
  ...
  // Create Space
  const createSpaceRes = await request('POST', '/api/manage/spaces', {
    name: 'Test Space',
    isPublic: true,
  });
  ...
  // Add File to Space
  const addFileSpaceRes = await request('POST', `/api/manage/spaces/${spaceId}/files`, {
    fileIds: [testFileId],
  });
  ...
  // Update Space
  const updateSpaceRes = await request('PUT', `/api/manage/spaces/${spaceId}`, {
    name: 'Updated Space',
  });
  ...
  const removeFileSpaceRes = await request('DELETE', `/api/manage/spaces/${spaceId}/files`, {
    fileIds: [testFileId],
  });
  ...
  // Update User
  const updateUserRes = await request('PUT', `/api/v1/users/${newUserId}`, {
    name: 'Updated User',
  });
  ...
  // Reset Token
  const resetTokenRes = await request(
    'POST',
    `/api/manage/salespersons/${salespersonId}/reset-token`
  );
  ...
  const getSalesOrderRes = await salesRequest(
    'GET',
    `/api/sales/${salespersonToken}/orders/${orderId}`
  );
  assertStatus(getSalesOrderRes, 200, 'Get Sales Order Detail failed');
  logSuccess('GET /api/sales/:token/orders/:id');

  // Sales Comment
  const salesCommentRes = await salesRequest(
    'POST',
    `/api/sales/${salespersonToken}/orders/${orderId}/comment`,
    {
  ...
  // Read Notification
  const readOrderRes = await salesRequest(
    'PATCH',
    `/api/sales/${salespersonToken}/orders/${orderId}/read`
  );
  ...
  // Update Order Info
  if (
    listAdminOrdersRes.data.data &&
    listAdminOrdersRes.data.data.orders &&
    listAdminOrdersRes.data.data.orders.length > 0
  ) {
    ...
    const updateOrderRes = await request('POST', `/api/manage/orders/${orderToUpdateId}/update`, {
      updates: {
        current_data: JSON.stringify({
          ...JSON.parse(listAdminOrdersRes.data.data.orders[0].currentData || '{}'),
  ...
  const deleteSalespersonFail = await request(
    'DELETE',
    `/api/manage/salespersons/${salespersonId}`
  );
  ...
  if (file3Res.status === 201) {
    const manageDeleteFileRes = await request(
      'DELETE',
      `/api/manage/files/${file3Res.data.data.id}`
    );
