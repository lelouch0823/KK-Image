...
describeIfRealApi('Webhook Real API', function () {
  ...
  itSkipInLoopback(
    'delivers supported procurement events with configured headers/signatures',
    async () => {
      const token = await getBearerToken();
  ...
  it('exposes only webhook-capable supported events and delivers file_uploaded events through the same outbox pipeline', async () => {
    ...
    try {
      ...
      assert.ok(
        supportedEvents.includes('file_uploaded'),
        'file_uploaded should be advertised as webhook-capable'
      );
      assert.ok(
        supportedEvents.includes('purchase_receipt_recorded'),
        'purchase_receipt_recorded missing from supported events'
      );
      assert.ok(
        !supportedEvents.includes('product_archived'),
        'cache-only product_archived should not be advertised'
      );
      assert.ok(
        !supportedEvents.includes('order_line_fulfillment_updated'),
        'cache-only line event should not be advertised'
      );
      ...
      const delivered = await receiver.waitForDelivery(
        (item) =>
          item.body?.event_type === 'file_uploaded' && item.body?.aggregate?.id === uploadedFileId,
        {
          timeoutMs: 15000,
          intervalMs: 500,
      ...
      assert.ok(
        delivered.headers['x-webhook-signature'],
        'file_uploaded webhook signature missing'
      );
  ...
  // loopback 模式下级联重启导致恢复超时，跳过此测试
  itSkipInLoopback(
    'retries a failed webhook delivery through the outbox poller and eventually publishes the job',
    async () => {
      const token = await getBearerToken();
      await cleanupTestManageWebhooks(token);
      ...
      const receiver = await startWebhookReceiver({
        ...
        responseResolver: ({ body }) => {
          const isTargetEvent =
            body?.event_type === 'purchase_receipt_recorded' &&
      ...
      try {
        ...
        await waitFor(
          async () => {
            ...
            const webhookJob = (detail.json?.data?.consumerJobs || []).find(
        ...
        const finalDetail = await waitFor(
          async () => {
        ...
        assert.ok(
          ...
          'target retry injection did not observe at least two deliveries'
        );
  ...
  // loopback 模式下级联重启导致恢复超时，跳过此测试
  itSkipInLoopback(
    'records terminal 4xx webhook failures without leaving the outbox job retryable',
    async () => {
      const token = await getBearerToken();
      await cleanupTestManageWebhooks(token);
      ...
      try {
        ...
        assert.ok(
          finalDetail.webhookAttempts.some((attempt) => attempt.delivery_key === deliveryKey)
        );
      } finally {
        if (webhookId) {
          try {
  ...
  // loopback 模式下级联重启导致恢复超时，跳过此测试
  itSkipInLoopback(
    'delivers reversal events through webhook subscriptions and persists published outbox detail for each delivery',
    async () => {
      const token = await getBearerToken();
      await cleanupTestManageWebhooks(token);
      ...
      try {
        ...
        const supportedEvents = listBefore.json?.supportedEvents || [];
        assert.ok(
          supportedEvents.includes('purchase_receipt_reversed'),
          'purchase_receipt_reversed missing from supported events'
        );
        assert.ok(
          supportedEvents.includes('inventory_receipt_reversed'),
          'inventory_receipt_reversed missing from supported events'
        );
        assert.ok(
          supportedEvents.includes('order_procurement_reversed'),
          'order_procurement_reversed missing from supported events'
        );
        ...
        for (const delivered of [
          ...
          assert.ok(
            delivered.headers['x-webhook-signature'],
            `${delivered.body?.event_type} webhook signature missing`
          );
        }
        ...
        assert.strictEqual(
          orderProcurementReversed.body?.payload?.order_procurement_status_after,
          'ordered'
        );

        const publishedDetails = await Promise.all([
          waitForPublishedWebhookOutboxEvent(
            token,
            purchaseReceiptReversed,
            'purchase_receipt_reversed'
          ),
