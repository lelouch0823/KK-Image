...
import {
  ...
  uniqueSeed,
  waitFor,
  shouldUseDirectSalesFlowTransport,
  withScopedRealApiTransport,
...
describeIfRealApi('Sales Order Collaboration Real API', function () {
  ...
  it('runs sales catalog -> sales order -> admin confirm -> procurement receipt -> delivery confirmation as one real business chain', async () => {
    await withScopedRealApiTransport(
      shouldUseDirectSalesFlowTransport() ? 'direct' : '',
      async () => {
        ...
        const listedVariant = (salesProductDetail.json?.data?.variants || []).find(
          (item) => item.id === variantId
        );
        assert.ok(listedVariant, 'sales product detail did not expose sellable variant');
        assert.ok(
          Number(listedVariant.available_quantity || 0) >= 5,
          'sales product detail available quantity missing'
        );
        ...
        await apiRequest(
          `/api/manage/orders/${orderId}/lines/${orderAfterReceipt.lines[0].id}/ship`,
          {
            bearerToken: token,
            method: 'POST',
            body: { quantity: 2 },
        ...
        const deliveredDetail = await waitFor(
          async () => {
            const detail = await salesApiRequest(
              salesSession.accessToken,
              salesSession.jwt,
            ...
            assert.ok(
              Number(detail.json?.data?.deliveryConfirmedAt || 0) > 0,
              'deliveryConfirmedAt missing'
            );
            return detail.json?.data;
          },
          {
        ...
        const deliveryNotification = await waitFor(
          async () => {
          ...
          },
          {
            ...
            intervalMs: 1000,
            onTimeoutMessage:
              'delivery confirmation notification did not reach sales notifications',
          }
        );
        assert.ok(
          String(deliveryNotification.title || '')
            .toLowerCase()
            .includes('delivery') ||
            String(deliveryNotification.content || '')
              .toLowerCase()
              .includes('delivery'),
      ...
      async () => {
        ...
        await waitFor(
          ...
          },
          {
            ...
            intervalMs: 500,
            onTimeoutMessage:
              'receipt baseline did not project into sales order detail before reversal',
          }
        );
