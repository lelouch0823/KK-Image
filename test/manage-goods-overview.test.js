...
describe('Goods Overview API', () => {
  ...
  describe('GET /api/manage/goods-overview', () => {
    ...
    it('should apply shortageOnly=1, sort=demand and filters', async () => {
      const res = await app.request(
        '/api/manage/goods-overview?shortageOnly=1&sort=demand&category=Cat1&brand=Brand1',
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
        localMockEnv,
    ...
    it('should apply sort=name', async () => {
      const res = await app.request(
        '/api/manage/goods-overview?sort=name',
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
        localMockEnv,
  ...
  describe('GET /api/manage/goods-overview/export', () => {
    it('should return CSV with BOM and data', async () => {
      const res = await app.request(
        '/api/manage/goods-overview/export',
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
        localMockEnv,
      ...
      assert.ok(
        res.headers.get('Content-Disposition').startsWith('attachment; filename="goods_overview_')
      );
