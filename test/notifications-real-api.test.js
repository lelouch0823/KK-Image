...
async function findSalesNotification(accessToken, authToken, predicate, {
  unreadOnly = false,
  limit = 20,
  cacheBust = false,
} = {}) {
  ...
  const query = `?${params.toString()}`;
  const result = await salesApiRequest(accessToken, authToken, `/api/sales/${accessToken}/notifications${query}`, {
    expectedStatus: 200,
  });
  const list = result.json?.data?.list || [];
  return {
...
describeIfRealApi('Notifications Real API Workflow', function () {
  ...
  it('materializes procurement notifications from business events and supports clearing unread state afterwards', async () => {
    ...
    await waitFor(async () => {
      ...
      assert.ok(!unreadResult.match, 'business notification still appears in unread list after mark-all-read');
      return unreadResult;
    }, {
      timeoutMs: 15000,
      intervalMs: 500,
      onTimeoutMessage: 'target business notification was not cleared from unread list after mark-all-read',
    });
  ...
  it('materializes sales notifications for admin-side order lifecycle events and supports sales read transitions', async () => {
    ...
    const createdNotification = await waitFor(async () => {
      ...
      assert.ok(result.match, 'order_created_by_admin sales notification has not been materialized yet');
      return result.match;
    }, {
    ...
    await waitFor(async () => {
      ...
      assert.ok(!unreadResult.match, 'sales created notification still appears in unread list after read');
      return unreadResult;
    }, {
      ...salesNotificationPoll,
    ...
    const lifecycleNotifications = await waitFor(async () => {
      ...
      assert.ok(updatedNotification, 'order_updated_by_admin sales notification has not been materialized yet');
      assert.ok(statusNotification, 'order_status_changed_by_admin sales notification has not been materialized yet');
      assert.ok(commentNotification, 'order_comment_created_by_admin sales notification has not been materialized yet');
    ...
    const markAllRead = await salesApiRequest(accessToken, jwt, `/api/sales/${accessToken}/notifications/all/read`, {
      method: 'POST',
      body: {},
      expectedStatus: 200,
    });
    ...
    await waitFor(async () => {
      ...
      assert.ok(!unreadResult.match, 'admin lifecycle sales notifications still appear in unread list after mark-all-read');
      return unreadResult;
    }, {
      ...salesNotificationPoll,
      onTimeoutMessage: 'admin lifecycle sales notifications were not cleared from unread list after mark-all-read',
    });
  });
  ...
  it('materializes admin notifications for sales-side order lifecycle events', async () => {
    ...
    const createdOrder = await salesApiRequest(accessToken, jwt, `/api/sales/${accessToken}/orders`, {
      method: 'POST',
      body: {
    ...
    const createdNotification = await waitFor(async () => {
      ...
      assert.ok(result.match, 'order_created_by_sales admin notification has not been materialized yet');
      return result.match;
    }, {
    ...
    const lifecycleNotifications = await waitFor(async () => {
      ...
      assert.ok(updatedNotification, 'order_updated_by_sales admin notification has not been materialized yet');
      assert.ok(commentNotification, 'order_comment_created_by_sales admin notification has not been materialized yet');
      assert.ok(statusNotification, 'order_status_changed_by_sales admin notification has not been materialized yet');
    ...
    await waitFor(async () => {
      ...
      assert.ok(!unreadResult.match, 'sales lifecycle admin notifications still appear in unread list after mark-all-read');
      return unreadResult;
    }, {
      ...adminNotificationPoll,
      onTimeoutMessage: 'sales lifecycle admin notifications were not cleared from unread list after mark-all-read',
    });
  });
  ...
  it('materializes deadline reminder notifications for both admin and sales through cron reminders', async () => {
    ...
    const adminReminder = await waitFor(async () => {
      const result = await findAdminNotification(
        token,
        (item) =>
      ...
      assert.ok(result.match, 'admin deadline reminder notification has not been materialized yet');
      return result.match;
    }, {
      timeoutMs: 20000,
      intervalMs: 500,
    ...
    const salesReminder = await waitFor(async () => {
      const result = await findSalesNotification(
        accessToken,
        jwt,
        (item) =>
      ...
      assert.ok(result.match, 'sales deadline reminder notification has not been materialized yet');
      return result.match;
    }, {
      ...salesNotificationPoll,
  ...
  it('materializes reversal notifications from procurement rollback events and supports clearing unread state afterwards', async () => {
    ...
    const reversalNotifications = await waitFor(async () => {
      ...
      assert.ok(purchaseReceiptReversed, 'purchase_receipt_reversed notification has not been materialized yet');
      assert.ok(orderProcurementReversed, 'order_procurement_reversed notification has not been materialized yet');
      assert.ok(result.unreadCount >= 2, 'unread count did not increase for reversal notifications');
    ...
    assert.ok(
      String(reversalNotifications.purchaseReceiptReversed.title || '').includes('notification.purchase_receipt_reversed')
    );
    assert.ok(
      String(reversalNotifications.purchaseReceiptReversed.content || '').includes(`"purchaseOrderId":"${poId}"`)
    );
    assert.ok(
    ...
    assert.ok(
      String(reversalNotifications.orderProcurementReversed.title || '').includes('notification.order_procurement_reversed')
    );
    assert.ok(
      String(reversalNotifications.orderProcurementReversed.content || '').includes(`"purchaseOrderId":"${poId}"`)
    );
    assert.ok(
      String(reversalNotifications.orderProcurementReversed.content || '').includes('"qty":2')
    );
    assert.ok(
      String(reversalNotifications.orderProcurementReversed.content || '').includes('"status":"ordered"')
    );
    ...
    await waitFor(async () => {
      ...
      assert.ok(!unreadResult.match, 'reversal notifications still appear in unread list after mark-all-read');
      return unreadResult;
    }, {
      timeoutMs: 15000,
      intervalMs: 500,
      onTimeoutMessage: 'target reversal notifications were not cleared from unread list after mark-all-read',
    });
    ...
    const finalList = await waitFor(async () => {
      ...
      assert.ok(purchaseReceiptReversed, 'purchase_receipt_reversed notification missing from full list after read');
      assert.ok(orderProcurementReversed, 'order_procurement_reversed notification missing from full list after read');
    ...
    assert.ok(
      String(finalList.purchaseReceiptReversed.title || '').includes('notification.purchase_receipt_reversed')
    );
    assert.ok(
