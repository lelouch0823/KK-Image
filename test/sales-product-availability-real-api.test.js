...
describeIfRealApi('Sales Product Availability Real API', function () {
  ...
  it('updates sales sellable catalog after confirmed order consumes stock and restores it after receipt', async () => {
    ...
    assert.ok(findSalesProduct(warmList.json, productId), 'sales warm list missing sellable product');
    ...
    const warmVariant = (warmDetail.json?.data?.variants || []).find((item) => item.id === variantId);
    ...
    await waitFor(async () => {
      ...
      assert.ok(!findSalesProduct(salesList.json, productId), 'sales catalog still exposed out-of-stock product');
      ...
      const variant = (salesDetail.json?.data?.variants || []).find((item) => item.id === variantId);
      assert.ok(!variant, 'sales product detail still exposed out-of-stock variant');
      return salesDetail.json?.data;
    }, {
      timeoutMs: 20000,
      intervalMs: 500,
      onTimeoutMessage: 'sales catalog did not hide exhausted sellable product after order confirmation',
    });
    ...
    await waitFor(async () => {
      ...
      const variant = (salesDetail.json?.data?.variants || []).find((item) => item.id === variantId);
      assert.ok(variant, 'sales product detail did not restore variant after receipt replenishment');
      assert.strictEqual(Number(variant.available_quantity || 0), 2);
      return salesDetail.json?.data;
    }, {
      timeoutMs: 20000,
      intervalMs: 500,
      onTimeoutMessage: 'sales catalog did not restore sellable product after receipt replenishment',
    });
  });
  ...
  it('hides the sales sellable catalog again after a restoring receipt is reversed', async () => {
    ...
    assert.ok(findSalesProduct(warmList.json, productId), 'sales warm list missing reversal product');
    ...
    await waitFor(async () => {
      ...
      assert.ok(!findSalesProduct(salesList.json, productId), 'sales catalog still exposed consumed product before reversal flow');
    ...
    await waitFor(async () => {
      ...
      assert.ok(!findSalesProduct(salesList.json, productId), 'sales catalog still exposed product after receipt reversal');
