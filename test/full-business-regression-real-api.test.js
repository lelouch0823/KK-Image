...
describeIfRealApi('Full Business Regression Real API', function () {
  ...
  it('runs file -> product -> order -> procurement -> notification in one workflow', async () => {
    ...
    await waitFor(async () => {
      ...
      assert.ok((movedFiles.json?.data?.files || []).some((item) => item.id === standaloneFileId), 'moved file missing from target folder');
    ...
    await waitFor(async () => {
      ...
      assert.ok(!(listAfterDelete.json?.data?.files || []).some((item) => item.id === standaloneFileId), 'deleted file still appears in folder listing');
    ...
    await waitFor(async () => {
      ...
      const variant = (variantDetail.json?.data?.variants || []).find((item) => item.id === variantId);
    ...
    await waitFor(async () => {
      const notification = await findAdminNotification(token, (item) => (
