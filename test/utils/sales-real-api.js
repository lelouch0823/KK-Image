...
export async function createSalespersonFixture(
  adminToken,
  seed,
  { password = '123456', namePrefix = 'Real API Sales', store = 'Real API Store' } = {}
) {
...
export async function loginSalesperson(accessToken, password = '123456') {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    const payload = await jsonRequest(`/api/sales/${accessToken}/auth`, {
      method: 'POST',
...
export async function salesApiRequest(
  accessToken,
  authToken,
  path,
  { method = 'GET', body, expectedStatus, headers: extraHeaders } = {}
) {
  void accessToken;
...
export async function salesMultipartRequest(
  path,
  { method = 'POST', fields = {}, authToken, expectedStatus, headers: extraHeaders } = {}
) {
