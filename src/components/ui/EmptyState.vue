<template>
  <div class="flex flex-col items-center justify-center py-12 text-center" :class="containerClass">
    ...
    >
      <slot name="icon">
        <svg class="w-8 h-8 text-gray-300" :class="iconClass" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Title -->
    <h3 class="text-lg font-medium text-primary" :class="titleClass">
      <slot name="title">{{ title }}</slot>
    </h3>

    <!-- Description -->
    <p v-if="description || $slots.description" class="text-secondary text-sm mt-1 max-w-sm" :class="descriptionClass">
