    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        <div class="px-6 pb-6 flex items-center gap-3">
          <button 
            @click="handleCancel"
            class="flex-1 py-2.5 px-4 text-sm font-semibold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-all active:scale-95"
          >
            {{ cancelText }}
          </button>
          <button 
            @click="handleConfirm"
            :class="[
              'flex-1 py-2.5 px-4 text-sm font-semibold text-white rounded-xl shadow-lg transition-all active:scale-95',
