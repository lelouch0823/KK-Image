<template>
  <ManagementListShell
    ...
    :description="t('product.manager.subtitle')"
  >
    <template #actions>
      <!-- Create Button -->
      <AppButton
    ...
    <!-- Import Modal -->
    <ProductImportModal v-model="showImportModal" @success="handleModalSuccess" />

    <!-- Export Modal -->
    <ProductExportModal v-model="showExportModal" :filters="filters" />
    ...
    <template #content>
      ...
      <div class="relative lg:min-h-[400px] lg:overflow-hidden">
        ...
        <div
          v-if="loading"
          class="absolute inset-0 z-10 flex items-center justify-center bg-(--bg-page)/50 backdrop-blur-[1px]"
        >
        ...
        <div
          ...
          class="custom-scrollbar hidden h-full overflow-auto lg:block"
        >
          <ProductTable
            :products="products"
...
const reloadProducts = (overrides = {}, forceRefresh = false) =>
  loadProducts(buildProductQuery(overrides), forceRefresh);
...
const handleQueryEditOpen = async (productId) => {
  ...
  try {
    ...
    queryEditError.value = t(
      'product.workflow.edit_load_failed',
...
const handleModalSuccess = async (createdProduct = null) => {
  ...
  await handleCreated({
    ...
    onHiddenByFilters: () => {
      addToast({
        message: t(
          'product.manager.createdHiddenByFilters',
          '商品已创建，但当前筛选条件未显示该项'
        ),
        type: 'info',
      });
