  ...
  <header class="h-[var(--header-height)] bg-white border-b border-[var(--border-color)] flex items-center justify-between px-4 lg:px-6 shrink-0">
    <div class="flex items-center gap-3">
      ...
      <button 
        ...
        class="lg:hidden p-2 -ml-2 rounded-lg hover:bg-gray-100 text-gray-600"
      >
    ...
    <div class="flex items-center gap-2 lg:gap-4">
      ...
      <div v-if="notificationsSupported" class="relative" ref="notificationRef">
        ...
        >
          ...
          <!-- 红点 -->
          <span v-if="unreadCount > 0" class="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border border-white"></span>
        </button>

        <!-- 下拉弹窗 -->
        <div 
          v-if="showNotifications"
          class="absolute right-0 top-full mt-2 z-50 origin-top-right transition-all duration-200"
        >
