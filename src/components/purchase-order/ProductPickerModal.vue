        ...
        <div class="relative flex w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-(--color-modal-bg) shadow-2xl" style="max-height: calc(100vh - 3rem)">
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-6 py-3">
            <div
              ...
              class="mb-3 rounded-xl border border-[var(--color-warning)]/30 bg-[var(--color-warning)]/10 px-3 py-2 text-xs text-[var(--text-main)]"
            >
              {{ t('purchaseOrder.selection.unavailableHint', '找不到的变体可能已被下架或归档') }} ({{ unavailableCount }})
            </div>

            <div v-if="errorMessage" role="alert" class="mb-3 rounded-xl border border-[var(--color-danger)]/30 bg-[var(--color-danger)]/10 px-3 py-2 text-sm text-[var(--text-main)]">
              <div class="flex items-center justify-between gap-3">
                <span>{{ errorMessage }}</span>
                <button type="button" class="cursor-pointer rounded-lg border border-[var(--border-color)] px-2 py-1 text-xs font-medium hover:bg-(--bg-hover)" @click="loadVariants">
