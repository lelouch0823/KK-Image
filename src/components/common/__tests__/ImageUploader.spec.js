...
describe('ImageUploader.vue', () => {
        ...
        return mount(ImageUploader, {
            ...
            global: {
                stubs: {
                    ...
                    UploadButton: {
                        name: 'UploadButton',
                        template: '<button data-testid="upload-button" @click="$emit(\'select\', $attrs.selectEvent)">upload</button>',
                    },
    ...
    it('adds instant-uploaded files when original hash precheck hits', async () => {
        ...
        expect(mockAddToast).toHaveBeenCalledWith({ message: 'upload.instantUploadSuccess', type: 'success' });
    });
