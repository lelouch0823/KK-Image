<template>
  <div class="border-success/20 bg-(--color-success-bg) rounded-2xl border p-4 shadow-card">
    <div class="flex items-start gap-3">
      <div
        class="text-success flex size-10 shrink-0 items-center justify-center rounded-xl bg-(--bg-card) shadow-sm"
      >
      ...
      <div class="min-w-0 flex-1">
        <div class="flex flex-wrap items-center gap-2">
          <p class="text-sm font-semibold text-(--text-main)">
            {{ t('common.ai.actionResult.created') }}
          </p>
          <span
    ...
    <div class="mt-4 grid gap-3 sm:grid-cols-2">
      <div class="rounded-xl bg-(--bg-card) p-3 shadow-sm">
        <p class="text-xs tracking-[0.08em] text-(--text-secondary) uppercase">
          {{ t('common.ai.actionResult.result') }}
        </p>
...
const successText = computed(
  () => props.action?.successMessage || t('common.ai.actionResult.defaultSuccess')
);
const entityLabel = computed(
  () =>
    props.action?.createdEntityLabel ||
    props.action?.createdEntityId ||
    t('common.ai.actionResult.defaultEntity')
);
const moduleLabel = computed(
  () => MODULE_LABELS.value[props.action?.targetModule] || t('common.ai.actionResult.defaultModule')
);
</script>
