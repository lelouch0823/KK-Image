  ...
  <div class="rounded-2xl border border-(--border-color) bg-(--bg-card) p-4 shadow-card">
    <div class="flex flex-wrap items-center gap-2">
      <span class="bg-primary/12 text-primary rounded-full px-2.5 py-1 text-xs font-medium">{{
        t('common.ai.slotQuestion.step1')
      }}</span>
      <span
      ...
      >
        {{
          candidateGroups.length > 0
            ? t('common.ai.slotQuestion.canSelect')
            : t('common.ai.slotQuestion.continueFilling')
        }}
      </span>
    </div>
    <p class="mt-3 text-sm font-semibold text-(--text-main)">
      {{ t('common.ai.slotQuestion.needMoreInfo') }}
    </p>
    ...
    <div v-if="candidateGroups.length > 0" class="mt-4 space-y-3">
      <div v-for="field in candidateGroups" :key="field.key" class="rounded-xl bg-(--bg-muted) p-3">
        <div class="flex items-center justify-between gap-3">
          <p class="text-xs font-medium text-(--text-main)">
            {{ field.label }} {{ t('common.ai.slotQuestion.candidates') }}
          </p>
          <span class="text-xs text-(--text-secondary)"
            >{{ field.candidates.length }} {{ t('common.ai.slotQuestion.items') }}</span
          >
        </div>
        <div class="mt-2 space-y-2">
          <AppButton
            ...
            :data-selected="
              selectedCandidates[field.key]?.value === candidate.value ? 'true' : 'false'
            "
            ...
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <p class="text-sm font-medium text-(--text-main)">
                  {{ index + 1 }}. {{ candidate.label || candidate.value }}
                </p>
                <p v-if="candidate.description" class="mt-1 text-xs text-(--text-secondary)">
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <p class="text-xs font-medium text-(--text-main)">
              {{ t('common.ai.slotQuestion.selected') }}
            </p>
            <AppButton
          ...
          <p class="mt-1 text-sm font-medium text-(--text-main)">
            {{ selectedCandidates[field.key].label || selectedCandidates[field.key].value }}
          </p>
          <p
            v-if="selectedCandidates[field.key].description"
        ...
        <p class="mt-2 text-xs text-(--text-secondary)">
          {{ t('common.ai.slotQuestion.replyHint') }}
        </p>
      </div>
    </div>
  </div>
...
const missingSlots = computed(() =>
  Array.isArray(props.action?.missingSlots) ? props.action.missingSlots : []
);
