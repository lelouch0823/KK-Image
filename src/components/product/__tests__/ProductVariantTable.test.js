...
describe('ProductVariantTable', () => {
  ...
  it('marks persisted stock inputs as readonly when inventory is managed externally', () => {
    const wrapper = mount(ProductVariantTable, {
      props: {
        modelValue: baseVariants,
  ...
  it('renders incomplete variants as pending warning rows', () => {
    ...
    expect(wrapper.find('[data-testid="variant-row-0"]').attributes('data-variant-state')).toBe(
      'pending_incomplete'
    );
