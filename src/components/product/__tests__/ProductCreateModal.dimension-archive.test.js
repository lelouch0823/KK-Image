...
describe('ProductCreateModal dimension archive mode', () => {
  beforeEach(() => {
    ...
    mocks.archiveDimension.mockResolvedValue({
      success: true,
      data: { effect: { archivedVariants: 1 } },
    });
  });
  ...
  it('uses merge_keep when user selects merge mode', async () => {
    ...
    expect(mocks.archiveDimension).toHaveBeenCalledWith('prod-1', 'dim-color', {
      mode: 'merge_keep',
    });
  });
  ...
  it('keeps the wizard open and clears loading when archiving a dimension rejects', async () => {
    ...
    await expect(
      wrapper.find('[data-testid="dimension-archive-confirm"]').trigger('click')
