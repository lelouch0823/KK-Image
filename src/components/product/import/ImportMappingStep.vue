<template>
  <div>
    ...
    <div class="mt-5 rounded-xl border border-(--border-color) bg-(--bg-muted) p-4">
      <div class="mb-3 flex items-center justify-between">
        <div>
          <h4 class="text-sm font-semibold text-(--text-main)">
            {{ t('product.import.specs.title', '规格配置') }}
          </h4>
          <p class="mt-1 text-xs text-(--text-secondary)">
            {{ t('product.import.specs.desc', '支持 0-3 个规格，支持预设名称或自定义名称。') }}
          </p>
        </div>
        <AppButton variant="ghost" size="sm" :disabled="specConfigs.length >= 3" @click="addSpec">
      ...
      <div
        v-for="(spec, index) in specConfigs"
        :key="spec.id || index"
        class="mb-3 rounded-lg border border-(--border-color) bg-(--bg-card) p-3 last:mb-0"
      >
        ...
        <div class="mb-2 flex flex-wrap gap-2">
          <AppButton
            ...
            :class="
              normalizeName(spec.name) === normalizeName(preset)
        ...
        <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <Select
            ...
            :options="[
              { label: t('product.import.specs.select_column', '选择规格列'), value: '' },
              ...fileHeaders.map((h) => ({ label: h, value: h })),
            ]"
    ...
    <div class="mt-4 rounded-xl border border-(--border-color) bg-(--bg-muted) p-4">
      <h4 class="text-sm font-semibold text-(--text-main)">
        {{ t('product.import.mode.title', '导入策略') }}
      </h4>
      <p class="mt-1 text-xs text-(--text-secondary)">
      ...
      <div class="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        >
          <p class="text-sm font-semibold">
            {{ t('product.import.mode.safe_merge', '仅更新无冲突（推荐）') }}
          </p>
          <p class="mt-1 text-xs text-(--text-secondary)">
            {{ t('product.import.mode.safe_merge_desc', '已有非空且不一致字段将跳过，并记录冲突') }}
          </p>
        </AppCard>
        <AppCard
          clickable
        ...
        >
          ...
          <p class="mt-1 text-xs text-(--text-secondary)">
            {{ t('product.import.mode.replace_desc', '同 SPU 命中后按导入值覆盖原有字段') }}
          </p>
        </AppCard>
      </div>
    </div>
    ...
    >
      <div class="mb-2 flex items-center justify-between">
        ...
        <span class="text-warning text-xs font-medium">
          {{
            t(
              'product.import.preprocess.report_total',
              { count: validationReport.total },
              '共 {count} 个问题'
            )
          }}
        </span>
      ...
      <div class="mb-3 flex flex-wrap gap-2 text-xs">
        <span class="border-warning/30 rounded-full border px-2 py-1">
          {{ t('product.import.preprocess.issue.missing_name', '商品名称为空') }}:
          {{ validationReport.byCode?.missing_name || 0 }}
        </span>
        <span class="border-warning/30 rounded-full border px-2 py-1">
          {{ t('product.import.preprocess.issue.missing_sku', 'SKU 为空') }}:
          {{ validationReport.byCode?.missing_sku || 0 }}
        </span>
        <span class="border-warning/30 rounded-full border px-2 py-1">
          {{ t('product.import.preprocess.issue.duplicate_sku', 'SKU 重复') }}:
          {{ validationReport.byCode?.duplicate_sku || 0 }}
        </span>
        <span class="border-warning/30 rounded-full border px-2 py-1">
          {{ t('product.import.preprocess.issue.empty_row', '整行为空或仅包含默认值') }}:
          {{ validationReport.byCode?.empty_row || 0 }}
        </span>
      </div>
...
const addSpec = () => {
  ...
  const next = [
    ...props.specConfigs,
    { id: `spec-${Date.now()}-${props.specConfigs.length}`, name: '', column: '' },
  ];
  updateSpecs(next);
};
...
const getPresetOptionsFor = (index) => {
  ...
  return SPEC_PRESETS.value.filter(
    (preset) =>
      !selected.has(normalizeName(preset)) ||
      normalizeName(props.specConfigs[index]?.name) === normalizeName(preset)
  );
};
</script>
