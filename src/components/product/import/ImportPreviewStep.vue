<template>
    ...
    <div v-if="parsedItems.length > 0" class="mt-4 border-t border-(--border-color) pt-3">
      <div class="flex items-center justify-between text-sm">
        <span class="text-(--text-secondary)"
          >{{ t('product.import.total_rows', '识别行数') }}:</span
        >
      ...
      <div
        v-if="hasSpu"
        class="bg-warning/10 text-warning mt-2 flex items-start gap-2 rounded px-2 py-1.5 text-xs"
      >
        <AppIcon name="exclamation-triangle" class="mt-0.5 size-3 shrink-0" />
        <span>{{
          t('product.import.spu_update_warning', '检测到相同 SPU 将更新原有商品及变体数据')
        }}</span>
      </div>

      <div
        v-if="parsedItems.length > 500"
        class="bg-warning/10 text-warning mt-2 rounded px-2 py-1 text-xs"
      >
  ...
  >
    ...
    <ul class="mt-2 list-disc space-y-1 pl-4 text-xs">
      <template v-if="importResult.summary">
        <li v-if="importResult.summary.createdProducts">
          {{ t('product.import.stats.created_products', '新建商品') }}:
          {{ importResult.summary.createdProducts }}
        </li>
        <li v-if="importResult.summary.updatedProducts">
          {{ t('product.import.stats.updated_products', '更新商品') }}:
          {{ importResult.summary.updatedProducts }}
        </li>
        <li v-if="importResult.summary.createdVariants">
          {{ t('product.import.stats.created_variants', '新建变体') }}:
          {{ importResult.summary.createdVariants }}
        </li>
        <li v-if="importResult.summary.updatedVariants">
          {{ t('product.import.stats.updated_variants', '更新变体') }}:
          {{ importResult.summary.updatedVariants }}
        </li>
        <li v-if="importResult.summary.conflicts">
          {{ t('product.import.stats.conflicts', '冲突并跳过') }}:
  ...
  <div
    v-if="
      importResult && Array.isArray(importResult.conflicts) && importResult.conflicts.length > 0
    "
    class="border-warning/30 bg-warning/10 mt-3 rounded-md border p-3"
  >
    <div class="mb-2 flex items-center justify-between">
      <p class="text-warning text-sm font-semibold">
        {{ t('product.import.conflicts.title', '冲突详情（已跳过）') }}
      </p>
          ...
          <thead class="bg-(--bg-muted)">
            <tr>
              <th class="px-2 py-1 text-left font-semibold">
                {{ t('product.import.conflicts.col_level', '层级') }}
              </th>
              <th class="px-2 py-1 text-left font-semibold">SPU</th>
              <th class="px-2 py-1 text-left font-semibold">SKU</th>
              <th class="px-2 py-1 text-left font-semibold">
                {{ t('product.import.conflicts.col_field', '字段') }}
              </th>
              <th class="px-2 py-1 text-left font-semibold">
                {{ t('product.import.conflicts.col_current', '原值') }}
              </th>
              <th class="px-2 py-1 text-left font-semibold">
                {{ t('product.import.conflicts.col_incoming', '导入值') }}
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            >
              ...
              <td class="truncate px-2 py-1" :title="String(conflict.current ?? '-')">
                {{ String(conflict.current ?? '-') }}
              </td>
              <td class="truncate px-2 py-1" :title="String(conflict.incoming ?? '-')">
  ...
  <div
    v-if="importError"
    class="mt-4 rounded-md bg-(--color-danger-bg) p-3 text-sm text-(--color-danger-text)"
  >
    {{ importError }}
  </div>
</template>
...
const showPreprocessStats = computed(() => Number(props.preprocessStats?.sourceRows || 0) > 0);
...
const normalizedConflictSearch = computed(() =>
  String(conflictSearch.value || '')
    .trim()
    .toLowerCase()
);
const visibleConflicts = computed(() => {
  ...
  return rows
    .filter(
      (row) =>
        conflictLevelFilter.value === 'all' ||
        String(row?.level || '') === conflictLevelFilter.value
    )
    .filter((row) => {
      if (!normalizedConflictSearch.value) return true;
      const haystack = [row?.spu, row?.sku, row?.field, row?.current, row?.incoming, row?.level]
...
const copyVisibleConflicts = async () => {
  const lines = visibleConflicts.value.map(
    (row) =>
