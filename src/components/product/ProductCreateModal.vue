<template>
  <Teleport to="body">
          ...
          <div class="custom-scrollbar max-h-[70vh] overflow-y-auto p-6">
              <form id="product-form" class="grid grid-cols-1 gap-8 lg:grid-cols-3" @submit.prevent="handleSubmit">
                  ...
                  <div class="space-y-6 lg:col-span-2">
                      ...
                      <!-- Options Builder -->
                      <div class="space-y-4 rounded-2xl border border-[var(--border-color)] bg-gradient-to-br from-[var(--bg-muted)] to-[var(--bg-card)] p-4">
                          <div class="flex items-center justify-between">
                              <div>
                                  <h4 class="font-bold text-[var(--text-main)]">{{ t('product.form.options_title', 'Product Options') }}</h4>
                                  <p class="text-xs text-[var(--text-secondary)]">{{ t('product.form.options_hint', '最多 3 个维度，支持归档与恢复') }}</p>
                              </div>
                              <div class="flex items-center gap-2">
                                  <button type="button" class="cursor-pointer rounded-lg border border-[var(--border-color)] px-2 py-1 text-xs font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-page)]" @click="showVariantBatchBuilder = true">
                                      {{ t('product.form.batch_build_variants', 'Batch Build') }}
                                  </button>
                                  <button type="button" class="flex cursor-pointer items-center gap-1 rounded-lg bg-[var(--color-primary)]/10 px-2 py-1 text-sm font-medium text-[var(--color-primary)] transition-colors hover:bg-[var(--color-primary)]/20 hover:text-[var(--color-primary-hover)]" @click="addOption">
                          ...
                          <div class="space-y-4">
                              <div v-for="(opt, idx) in form.options" :key="idx" class="relative rounded-xl border border-[var(--border-color)]/70 bg-[var(--bg-card)] p-3 shadow-sm transition-shadow hover:shadow-md">
                                  <button type="button" class="absolute top-2 right-2 cursor-pointer text-[var(--text-muted)] transition-colors hover:text-[var(--color-danger)]" @click="removeOption(idx)">
                                  ...
                                  <div class="mb-2 flex items-center gap-2">
                                      <span class="rounded-full bg-[var(--bg-muted)] px-2 py-0.5 text-[10px] font-semibold text-[var(--text-secondary)]">{{ t('product.form.dimension_label', '维度') }} {{ idx + 1 }}</span>
                                      <span class="rounded-full bg-[var(--color-primary)]/10 px-2 py-0.5 text-[10px] font-semibold text-[var(--color-primary)]">{{ opt.values.length }} {{ t('product.form.values_count', '值') }}</span>
                                  </div>
                                  ...
                                  <div>
                                      ...
                                      <div class="mt-2 flex flex-wrap gap-2">
                                          <span v-for="(val, vIdx) in opt.values" :key="vIdx" class="inline-flex items-center gap-1 rounded-full border border-[var(--border-color)] bg-[var(--bg-muted)] px-2.5 py-0.5 text-xs font-medium text-[var(--text-main)]">
                                              {{ val }}
                                              <button type="button" class="text-[var(--text-muted)] hover:text-[var(--color-danger)]" @click="removeOptionValue(opt, vIdx)">&times;</button>
                                          </span>
                                      </div>
                                      <div v-if="Array.isArray(opt.archivedValues) && opt.archivedValues.length > 0" class="mt-2 flex flex-wrap items-center gap-2">
                                          <span class="text-[10px] text-[var(--text-secondary)]">{{ t('product.form.archived_values', 'Archived') }}:</span>
                                          <span
                              ...
                              <table class="w-full text-left text-sm whitespace-nowrap">
                                  <thead class="text-xs text-[var(--text-secondary)] uppercase">
                                      <tr>
                                          ...
                                          <th class="w-32 px-3 py-2">SKU</th>
                                          <th class="w-36 px-3 py-2">Barcode</th>
                                          <th class="w-36 px-3 py-2">Supplier SKU</th>
                                          <th class="w-28 px-3 py-2">Price</th>
                                          <th class="w-28 px-3 py-2">Cost</th>
                                          <th class="w-24 px-3 py-2">Stock</th>
                                          <th class="w-24 px-3 py-2">Alert</th>
                                          <th class="w-24 px-3 py-2">Status</th>
                                          <th class="px-3 py-2">Images</th>
                                      </tr>
                                  </thead>
                                  <tbody class="divide-y divide-[var(--border-color)]/30">
                                      <tr v-for="(variant, idx) in form.variants" :key="idx" class="transition-colors hover:bg-[var(--bg-card)]/50">
                                          <td class="px-3 py-2 font-medium text-[var(--text-main)]">{{ formatVariantName(variant.options_values) }}</td>
                                          <td class="px-3 py-2"><input v-model="variant.sku" type="text" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs"></td>
                                          <td class="px-3 py-2"><input v-model="variant.barcode" type="text" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs" placeholder="Barcode"></td>
                                          <td class="px-3 py-2"><input v-model="variant.supplier_sku" type="text" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs" placeholder="Supplier SKU"></td>
                                          <td class="px-3 py-2"><input v-model.number="variant.price" type="number" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs"></td>
                                          <td class="px-3 py-2"><input v-model.number="variant.cost_price" type="number" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs"></td>
                                          <td class="px-3 py-2"><input v-model.number="variant.stock_quantity" type="number" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs"></td>
                                          <td class="px-3 py-2"><input v-model.number="variant.alert_threshold" type="number" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs"></td>
                                          <td class="px-3 py-2">
                                            <select v-model="variant.status" class="input w-full border-[var(--border-color)] bg-[var(--bg-card)] p-1 text-xs">
    ...
    <Modal
      ...
      @update:model-value="(value) => { if (!value) closeDimensionArchiveWizard(); }"
    >
      <div data-testid="dimension-archive-modal" class="relative">
        <h4 class="text-lg font-bold text-[var(--text-main)]">{{ t('product.form.archive_dimension_title', 'Archive Dimension') }}</h4>
        <p class="mt-2 text-xs text-[var(--text-secondary)]">{{ t('product.form.step_label', 'Step') }} {{ dimensionArchiveWizard.step }} / 2</p>
        <div v-if="dimensionArchiveWizard.step === 1">
          <p class="mt-2 text-sm text-[var(--text-secondary)]">
            {{ t('product.form.archive_dimension_impact', 'This action will affect variants:') }}
            <span class="font-semibold text-[var(--text-main)]">{{ dimensionArchiveWizard.affectedVariantsCount }}</span>
          </p>
          <p class="mt-1 text-xs text-[var(--text-secondary)]">
            {{ t('product.form.archive_dimension_hint', 'Preview impact first, then choose action strategy.') }}
          </p>
          <div v-if="dimensionArchiveWizard.sampleVariants.length > 0" class="mt-3 rounded-lg border border-[var(--border-color)] bg-[var(--bg-page)] p-2">
            <p class="mb-1 text-[10px] font-semibold text-[var(--text-secondary)]">{{ t('product.form.sample_variants', 'Sample variants') }}</p>
        ...
        <div v-else class="mt-4 space-y-2">
          <label class="flex cursor-pointer items-start gap-2 rounded-lg border border-[var(--border-color)] p-3">
            <input
            ...
            >
            <div>
              <p class="text-sm font-medium text-[var(--text-main)]">{{ t('product.form.archive_affected_variants', 'Archive affected variants') }}</p>
              <p class="text-xs text-[var(--text-secondary)]">{{ t('product.form.archive_affected_variants_desc', 'Safe default for edit flow.') }}</p>
            </div>
          </label>
          <label class="flex cursor-pointer items-start gap-2 rounded-lg border border-[var(--border-color)] p-3">
            <input
            ...
            >
            <div>
              <p class="text-sm font-medium text-[var(--text-main)]">{{ t('product.form.merge_and_keep', 'Merge & keep') }}</p>
              <p class="text-xs text-[var(--text-secondary)]">{{ t('product.form.merge_and_keep_desc', 'Ignore removed dimension and dedupe by signature.') }}</p>
            </div>
          </label>
        </div>
        ...
        <div class="mt-5 flex justify-end gap-2">
          ...
          >
            {{ dimensionArchiveWizard.loading ? t('common.processing', 'Processing...') : t('common.confirm', 'Confirm') }}
          </button>
        </div>
      </div>
    </Modal>
    <Modal
      ...
      @update:model-value="(value) => { if (!value) closeValueArchiveWizard(); }"
    >
      <div data-testid="value-archive-modal" class="relative">
        <h4 class="text-lg font-bold text-[var(--text-main)]">{{ t('product.form.archive_value_title', 'Archive Value') }}</h4>
        <p class="mt-2 text-sm text-[var(--text-secondary)]">
          {{ t('product.form.archive_value_impact', 'This action will affect variants:') }}
          <span class="font-semibold text-[var(--text-main)]">{{ valueArchiveWizard.affectedVariantsCount }}</span>
        </p>
        <p class="mt-1 text-xs text-[var(--text-secondary)]">
          {{ t('product.form.archive_value_hint', 'Archived values will not be used for new combinations.') }}
        </p>
        <div v-if="valueArchiveWizard.sampleVariants.length > 0" class="mt-3 rounded-lg border border-[var(--border-color)] bg-[var(--bg-page)] p-2">
          <p class="mb-1 text-[10px] font-semibold text-[var(--text-secondary)]">{{ t('product.form.sample_variants', 'Sample variants') }}</p>
          <div class="flex flex-wrap gap-1.5">
            <span
        ...
        <div class="mt-5 flex justify-end gap-2">
          ...
          >
            {{ valueArchiveWizard.loading ? t('common.processing', 'Processing...') : t('common.confirm', 'Confirm') }}
          </button>
        </div>
      </div>
    </Modal>
...
const removeOption = async (idx) => {
    ...
    if (props.editMode && option.id && props.initialData?.id) {
        ...
        dimensionArchiveWizard.sampleVariants = Array.isArray(impact?.data?.sampleVariants) ? impact.data.sampleVariants : [];
...
const removeOptionValue = async (opt, vIdx) => {
    ...
    if (props.editMode && opt.id && props.initialData?.id && value) {
        ...
        if (valueMeta?.id) {
            ...
            valueArchiveWizard.sampleVariants = Array.isArray(impact?.data?.sampleVariants) ? impact.data.sampleVariants : [];
            return;
        }
    }
...
const formatVariantSample = (sample) => {
    ...
    const parts = Object.values(optionsValues || {}).map((value) => String(value || '').trim()).filter(Boolean);
