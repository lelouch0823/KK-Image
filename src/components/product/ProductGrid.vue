  ...
  <div class="space-y-3">
    ...
    >
      <div class="flex items-start gap-3 p-3">
        ...
        <div
          class="size-20 shrink-0 overflow-hidden rounded-xl border border-(--border-color) bg-(--bg-muted)"
        >
          <AppImage
          ...
          >
            <template #placeholder>
              <div
                class="flex size-full items-center justify-center bg-(--bg-muted) text-(--text-secondary)/30"
              >
        ...
        <div class="min-w-0 flex-1">
          <!-- Title -->
          <h3
            class="truncate text-[15px] leading-tight font-medium text-(--text-main)"
            :title="product.name || '-'"
          >
            {{ product.name }}
          </h3>
          ...
          <div class="mt-1.5 flex min-w-0 flex-wrap items-center gap-1.5">
            <span
              class="max-w-[8.5rem] truncate rounded bg-(--bg-muted) px-1.5 py-0.5 font-mono text-xs text-(--text-secondary)"
              :title="product.spu || '-'"
            >
              {{ product.spu }}
            </span>
            <span
              class="max-w-[8rem] truncate text-xs text-(--text-secondary)"
              :title="`${t('product.table.header.stock')}: ${resolveDisplayStock(product)}`"
            >
              {{ t('product.table.header.stock') }}: {{ resolveDisplayStock(product) }}
            </span>
            <span
              v-if="resolveDisplayStock(product) <= resolveAlertThreshold(product)"
              class="bg-danger-bg text-danger-text rounded px-1.5 py-0.5 text-xs font-bold"
            >
