    ...
    <template #content>
      ...
      >
        ...
        <transition name="fade">
          ...
          >
            <div class="mb-4 animate-bounce rounded-full bg-(--bg-card) p-6 shadow-lg">
              <AppIcon name="cloud-arrow-up" class="text-info size-12" />
            </div>
        ...
        <div v-else-if="error" class="flex flex-1 flex-col items-center justify-center p-6">
          <EmptyState icon="inbox" :title="t('common.error')" :description="error" type="danger">
            <template #action>
              <AppButton
                variant="primary"
                :text="t('common.retry')"
        ...
        >
          ...
          <div
            ...
            class="flex-1 px-4 pt-0 pb-4 lg:px-6 lg:pb-6"
          ...
          <EmptyState
            ...
            :description="
              searchQuery
                ? t('common.noResultsDesc', { query: searchQuery })
                : t('fileManager.emptyDesc')
            "
          >
...
const { selectedIds, toggleSelect, selectAll, clearSelection } = useFileSelection(displayedFiles);
...
const { isDragging, onDragEnter, onDragLeave, onDragOver, onDrop } = useFileDrag(
  currentFolder,
  handleFilesDropped
);
...
const Icons = {
  ...
  grid: createIcon(
    'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z'
  ),
  open: createIcon(
...
const openBackgroundContextMenu = (e) => {
  const menuItems = [
    ...
    {
      label:
        viewMode.value === 'list' ? t('fileManager.viewMode.grid') : t('fileManager.viewMode.list'),
  ...
  if (canWriteFiles.value || canManageFolders.value) {
    menuItems.splice((canWriteFiles.value ? 1 : 0) + (canManageFolders.value ? 1 : 0), 0, {
      type: 'separator',
    });
  }
...
const openContextMenu = (e, item, type) => {
  ...
  if (type === 'file') {
    if (canWriteFiles.value) {
      menuItems.push({
        label: t('fileManager.contextMenu.rename'),
        icon: Icons.rename,
        action: () => modals.value.openRename({ id: item.id, type, name: item.name }),
      });
    }
    if (canMoveFiles.value) {
      menuItems.push({
        label: t('fileManager.contextMenu.move'),
        icon: Icons.move,
        action: () => handleMoveFile(item),
      });
    }
    menuItems.push({
      label: t('fileManager.contextMenu.share'),
      icon: Icons.share,
      action: () => handleShareFile(item),
    });
    menuItems.push({
    ...
    menuItems.push({
      label: t('fileManager.contextMenu.move'),
      icon: Icons.move,
      action: () => handleMoveFolder(item),
    });
    menuItems.push({
      label: t('fileManager.contextMenu.share'),
      icon: Icons.share,
      action: () => openShareFolderModal(item),
    });
  }
