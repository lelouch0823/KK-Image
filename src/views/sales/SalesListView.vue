<template>
  ...
  >
    <!-- Search Bar -->
    <div
      class="sticky top-14 z-20 -mx-4 mb-4 border-b border-(--border-color) bg-(--bg-page) px-4 py-3 transition-[background-color,box-shadow,border-radius] duration-200 sm:top-20 sm:mx-0 sm:rounded-xl sm:border sm:bg-(--bg-card) sm:shadow-card"
    >
      <SearchInput
        v-model="searchQuery"
    ...
    <div class="pb-20">
      ...
      ></div>
      <div
        v-else-if="orders.length > 0 && !infiniteScroll.canLoadMore.value"
