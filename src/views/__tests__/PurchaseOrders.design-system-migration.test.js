...
describe('PurchaseOrders design-system migration', () => {
  ...
  it('replaces the heavy overview hero gradient with quieter panel framing', () => {
    ...
    expect(source).toContain(
      'rounded-[1.75rem] border border-(--border-color)/60 bg-(--bg-card) p-4'
    );
  ...
  it('avoids feature-card gradients inside the detail workspace', () => {
    ...
    const detailWorkspace =
      detailStart >= 0 && detailEnd > detailStart ? source.slice(detailStart, detailEnd) : '';
    ...
    expect(detailWorkspace).not.toContain(
      'bg-linear-to-r from-sky-50/75 via-(--bg-card) to-amber-50/40'
    );
    expect(detailWorkspace).not.toContain(
      'bg-linear-to-br from-(--bg-card) via-(--bg-card) to-sky-50/35'
    );
    expect(detailWorkspace).not.toContain(
      'bg-linear-to-br from-(--bg-card) via-(--bg-card) to-amber-50/45'
    );
    expect(detailWorkspace).not.toContain(
      'bg-linear-to-r from-(--bg-card) via-(--bg-card) to-sky-50/30'
    );
    expect(detailWorkspace).not.toContain(
      'bg-linear-to-r from-(--bg-card) via-(--bg-card) to-emerald-50/28'
    );
  ...
  it('uses ledger-style toolbar structure and copy for the table context', () => {
    ...
    expect(toolbarBlock).toContain(
      '<h3 class="text-sm font-semibold text-(--text-main)">Order Ledger</h3>'
    );
    expect(toolbarBlock).toContain("'purchaseOrder.ui.tableHint'");
    expect(toolbarBlock).toContain("'主状态和到货进度在同一列聚合展示，便于快速扫读链路卡点。'");
  ...
  it('drops the table-row cost pill so money reads as calmer ledger data', () => {
    const source = readFileSync(resolve(process.cwd(), 'src/views/PurchaseOrders.vue'), 'utf8');
    const totalCostCell =
      source.match(/<template #cell-total_goods_cost="\{ row: po \}">[\s\S]*?<\/template>/)?.[0] ||
      '';
    ...
    expect(source).toContain('data-testid="purchase-order-total-cost"');
    expect(totalCostCell).toContain(
      'inline-flex min-w-[7.5rem] justify-end font-mono text-sm font-semibold text-(--text-main) tabular-nums'
    );
  ...
  it('removes dead top-level overview state from the script', () => {
    ...
    const consoleSignalsBlock = source.match(
      /const consoleSignals = computed\(\(\) => \{[\s\S]*?\n\}\);\n\nconst detailSummaryCards = computed/
    );
