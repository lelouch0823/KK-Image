...
export function renderMarkdown(content: string): string {
    ...
    const AI_INTERNAL_TAG = /<\/?(tools|call|arg_key|arg_value|function_name|parameters|tool_code|thought|think|reasoning)[^>]*>/gi;
    processed = processed.replace(AI_INTERNAL_TAG, '');


    // === 预处理 1：修复加粗/斜体标记 ===
    // AI 可能产生带空格或换行的加粗标记 (e.g. ** text ** -> **text**)
    processed = processed.replace(/\*\*\s*([\s\S]*?)\s*\*\*/g, (_: string, p1: string) => `**${p1.trim()}**`);
    processed = processed.replace(/\*\s*([^\s*][^*]*?)\s*\*/g, (_: string, p1: string) => `*${p1.trim()}*`);
