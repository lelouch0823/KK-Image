...
export class SSEParser {
    constructor() {
        /** @type {string} 内部缓冲区，用于存储尚未构成完整事件的片段 */
        this.buffer = '';
    }

    /**
     * 将新接收到的文本块推入解析器。
     * @param {string} chunk - 从流中接收到的文本块。
     * @returns {Array<{type: string, data: any}>} - 解析出的完整事件数组。
     */
    feed(chunk) {
        this.buffer += chunk;
        const events = [];
    ...
    /**
     * 解析单个事件数据块。
     * @param {string} text - 原始事件文本块。
     * @returns {Object|null} - 解析后的事件对象，如果格式不正确则返回 null。
     */
    parseEvent(text) {
