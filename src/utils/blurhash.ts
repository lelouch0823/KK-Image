 ...
 * @param width - 输出图像宽度 (推荐 32)
 * @param height - 输出图像高度 (推荐 32)
 * @param punch - 颜色增强系数 (默认 1)
 * @returns RGBA 像素数据
 */
export function decode(blurhash: string, width: number, height: number, punch: number = 1): Uint8ClampedArray {
 ...
 * @param blurhash - Blurhash 字符串
 * @param width - 输出宽度 (默认 32)
 * @param height - 输出高度 (默认 32)
 * @returns Data URL
 */
export function blurhashToDataURL(blurhash: string, width: number = 32, height: number = 32): string {
