...
interface CreateProductFormSubmitHandlerOptions {
  form: ProductForm;
  imageObjects: { value: ImageObject[] };
  editMode: { value: boolean };
  initialData: { value: Record<string, unknown> | null };
  t: (key: string, fallback?: string) => string;
  addToast: (options: { type: string; message: string }) => void;
  emit: (event: string, ...args: unknown[]) => void;
  incompleteVariantCount: { value: number };
  submitting: { value: boolean };
  nextSubmitRequestId: (() => number) & { current: () => number };
  isSubmitActionActive: (requestId: number) => boolean;
  createProduct: (payload: Record<string, unknown>) => Promise<unknown>;
  updateProduct: (id: string, payload: Record<string, unknown>) => Promise<unknown>;
  createProductWithMeta?: (payload: Record<string, unknown>) => Promise<unknown>;
  updateProductWithMeta?: (id: string, payload: Record<string, unknown>) => Promise<unknown>;
  formatSubmittedCurrency: (value: unknown) => string;
  isExistingVariantInEditMode: (editModeRef: { value: boolean }, variant: { id?: string }) => boolean;
  buildVariantSyncSummaryMessage: (sync: VariantSync, translate: unknown) => string;
  resolveActionErrorMessage: (error: unknown) => string;
}

export const createActionErrorMessageResolver = ({ t }: { t: (key: string, fallback?: string) => string }) => (error: unknown): string =>
  (error as Error)?.message || (error as Record<string, string>)?.error || t('common.operationFailed');

export const normalizeProductFormMutationResult = (result: unknown): MutationResult => {
...
}: CreateProductFormSubmitHandlerOptions) {
  return async function handleSubmit(): Promise<void> {
    if (!form.name) {
      addToast({
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        variants: form.variants.map((variant) => {
          ...
          const payloadVariant: Record<string, unknown> = {
            ...variantPayload,
