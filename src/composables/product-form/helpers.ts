...
export function formatSubmittedCurrency(value: unknown): string {
  const code = String(value || '').trim().toUpperCase();
  return code || 'CNY';
}

export function isExistingVariantInEditMode(editModeRef: { value: boolean }, variant: { id?: string }): boolean {
  return Boolean(editModeRef?.value && variant?.id);
}

function normalizeOptionKeys(optionsValues: Record<string, unknown>): string[] {
...
export function detectIncompleteVariant(activeDimensionNames: string[] = [], variant: Record<string, unknown> = {}, isEditMode = false): boolean {
  ...
  const variantKeys = normalizeOptionKeys((variant.options_values || {}) as Record<string, unknown>);
...
function translateWithFallback(translate: unknown, key: string, params: Record<string, unknown>, fallback: string): string {
  const resolved = typeof translate === 'function' ? translate(key, params) : '';
  if (!resolved || resolved === key) return fallback;
  return resolved;
}

export function buildVariantSyncSummaryMessage(sync: Record<string, unknown> = {}, translate: unknown): string {
