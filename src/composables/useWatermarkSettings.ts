...
interface WatermarkRawSettings {
  WATERMARK_ENABLED: string;
  WATERMARK_TEXT: string;
  WATERMARK_POSITION: string;
  WATERMARK_OPACITY: string;
  WATERMARK_COLOR: string;
  WATERMARK_SIZE_RATIO: string;
}

interface WatermarkParsedSettings {
  enabled: boolean;
  text: string;
  position: string;
  opacity: number;
...
const watermarkSettings: Ref<WatermarkRawSettings> = ref({
...
export function useWatermarkSettings() {
    ...
    const loadSettings = async (force = false): Promise<WatermarkRawSettings> => {
    ...
    const getSettingsParsed = (): WatermarkParsedSettings => {
        return {
