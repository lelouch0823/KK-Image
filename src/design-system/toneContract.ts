export const CANONICAL_TONES: readonly string[] = Object.freeze([
  'neutral',
  'primary',
  'success',
  'warning',
...
const LEGACY_TONE_ALIASES: Readonly<Record<string, string>> = Object.freeze({
  default: 'neutral',
  slate: 'neutral',
  blue: 'info',
  teal: 'info',
...
const TONE_CLASS_SETS: Readonly<Record<string, Record<string, string>>> = Object.freeze({
  neutral: {
...
const STATUS_TONE_MAP: Readonly<Record<string, string>> = Object.freeze({
  pending: 'warning',
  confirmed: 'info',
  production: 'primary',
  shipping: 'info',
...
export function normalizeTone(tone: string = 'neutral'): string {
  if (!tone) return 'neutral';
  if (CANONICAL_TONES.includes(tone)) return tone;
  return LEGACY_TONE_ALIASES[tone] || 'neutral';
}

export function getToneClasses(tone: string = 'neutral'): Record<string, string> {
