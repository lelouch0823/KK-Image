...
2025 年 12 月--多存储架构与代码优化

- **🚀 多存储架构**
  - 支持 Cloudflare R2、Telegram、S3 兼容服务三种存储后端
  - **智能路由**: 根据文件大小/类型自动选择最优存储
  - **冗余备份**: 支持同时存储到多个后端
  - **智能回退**: 主存储失败时自动切换备用存储
  - Telegram 配置改为可选，默认使用 R2

- **🔐 安全增强**
  - 密码哈希升级为 PBKDF2（100,000 次迭代）
  - JWT 使用 URL-safe Base64 编码（RFC 4648）
  - 时序安全比较使用 Cloudflare 原生 `timingSafeEqual` API
  - R2 操作符合官方最佳实践（`writeHttpMetadata`、`onlyIf`）

- **⚡ 性能优化**
  - 统计 API 多次遍历合并为单次遍历
  - 权限展开结果缓存（60 秒 TTL）
  - 后台镜像任务使用 `waitUntil` 正确调度

- **🛠️ 代码质量**
  - 新增类型化错误处理模块（`errors.js`）
  - 统一 API 响应格式为 JSON
  - 添加完整 JSDoc 类型注释
  - 删除 API 调用改用 DELETE 方法

- **📚 文档更新**
  - 新增安全架构文档 (`docs/architecture/security.md`)
  - 重写环境变量文档，添加多存储配置指南
  - 更新 `wrangler.toml` 配置结构
