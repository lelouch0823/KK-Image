...
CREATE TABLE IF NOT EXISTS erp_connections (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  adapter_type TEXT NOT NULL,               -- 适配器类型: 'generic', 'kingdee', 'yonyou', 'sap'
  base_url TEXT NOT NULL,                   -- ERP 系统 API 地址
  auth_type TEXT NOT NULL DEFAULT 'api_key', -- 认证方式: 'api_key', 'oauth2', 'basic'
  credentials TEXT NOT NULL DEFAULT '{}',   -- 加密存储的凭据 JSON
  config TEXT NOT NULL DEFAULT '{}',        -- 适配器特定配置 JSON
  sync_direction TEXT NOT NULL DEFAULT 'bidirectional', -- 'push', 'pull', 'bidirectional'
...
CREATE TABLE IF NOT EXISTS erp_sync_logs (
  ...
  status TEXT NOT NULL DEFAULT 'pending',   -- 'pending', 'success', 'failed', 'conflict'
...
CREATE INDEX IF NOT EXISTS idx_erp_entity_mappings_local ON erp_entity_mappings(connection_id, entity_type, local_id);
CREATE INDEX IF NOT EXISTS idx_erp_entity_mappings_erp ON erp_entity_mappings(connection_id, entity_type, erp_id);
