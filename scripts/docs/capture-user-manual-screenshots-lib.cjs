...
async function resolveDocFixtures(request, options = {}) {
...
function createCaptureUserManualScreenshotsRunner(options = {}) {
  ...
  async function main() {
    ...
    try {
      ...
      const fixtures = await resolveFixtures(adminContext.request);
      ...
      await loginPage.screenshot({
        path: pathModule.join(screenshotDir, '01-admin-login.png'),
        fullPage: false,
        caret: 'hide',
      });
...
async function runCaptureUserManualScreenshotsCli(options = {}) {
  return createCaptureUserManualScreenshotsRunner(options).main();
}
