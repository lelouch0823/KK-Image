...
describe('runUiFoundationUsageCheck', () => {
  ...
  it('passes when remediated files use shared foundation components only', () => {
    ...
    makeFile(
      ...
      '<template><AppButton>AI</AppButton></template>'
    );
  ...
  it('reports raw svg, button, and form-control violations', () => {
    makeFile(
      ...
      '<template><svg /><button>Legacy</button></template>'
    );
    makeFile(
      rootDir,
      'src/components/common/AIChatWidget.vue',
      '<template><button>Ask</button></template>'
    );
    ...
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining(
        '[no-raw-svg-in-remediated-web-files] src/views/Dashboard.vue:1 contains <svg'
      )
    );
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining(
        '[no-raw-buttons-in-remediated-web-files] src/views/Dashboard.vue:1 contains <button'
      )
    );
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining(
        '[no-raw-buttons-in-remediated-web-files] src/components/common/AIChatWidget.vue:1 contains <button'
      )
    );
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining(
        '[no-raw-form-controls-in-remediated-web-files] src/components/ShareFileModal.vue:1 contains <input'
      )
    );
  });
});
