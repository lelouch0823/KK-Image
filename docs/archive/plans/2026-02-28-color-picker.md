...
<div>
  <div class="flex items-center gap-2">
    ...
    <div
      ...
      class="relative size-8 shrink-0 overflow-hidden rounded-md border border-(--border-color)"
    >
      <input
        type="color"
  ...
  <div class="mt-3 flex flex-wrap gap-2">
    ...
    >
      ...
      >
        <input type="color" v-model="opt.metaMap[val].color" class="absolute opacity-0 size-0" />
      </label>
...
const buildColorSwatchStyle = (value) => {
  ...
  const colorDimension = (props.boundProduct?.dimensions || []).find(
    (d) =>
      typeof d.name === 'string' &&
      (d.name.toLowerCase().includes('color') || d.name.includes('色'))
  );
