...
export class ProductVariantRepository {
      ...
      statements.push(
        this.db
          ...
          .bind(
            ...
            v.sku,
            Number(v.price) || 0,
            v.cost_price ? Number(v.cost_price) : null,
  ...
  async adjustStock(variantId, delta) {
    ...
    const result = await this.db
      ...
      .bind(delta, timestamp, variantId)
      .run();
...
_In `functions/lib/hono/routes/manage/products/index.js` (POST handler):_
...
_In `functions/lib/hono/routes/manage/products/[id].js` (GET handler):_
...
_In `ProductCreateModal.vue` and `ProductDetailModal.vue`:_
