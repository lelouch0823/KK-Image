...
SYSTEM_FIELDS.forEach((field) => {
  ...
  const match = headers.find(
    (h) => String(h).toLowerCase().includes(field.key) || String(h).includes(field.label)
  );
...
- **Logic**:
  - Query D1 for _all_ products (cursor/pagination loop to avoid memory limits).
...
5.  **Step 5 Submit**:
    ...
    - Replaces the filename in the payload with the returned _Remote R2 URL_.
