...
export function createActionSubmitters(deps = {}) {
  return {
    async create_customer(slots = {}) {
      ...
      const payload = pickDefined(slots, [
        'name',
        'phone',
        'company',
        'email',
    ...
    async create_purchase_order(slots = {}) {
      ...
      if (mode === 'from_orders') {
        if (!deps.purchaseOrderService?.createFromOrders)
          throw new Error('Purchase order from-orders dependency is unavailable');
      ...
      if (!deps.purchaseOrderRepo?.create)
        throw new Error('Purchase order create dependency is unavailable');
    ...
    async create_salesperson(slots = {}) {
      if (!deps.salespersonRepo?.create)
        throw new Error('Salesperson create dependency is unavailable');
