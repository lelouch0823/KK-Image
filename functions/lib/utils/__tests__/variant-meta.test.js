...
describe('variant-meta (backend)', () => {
  ...
  it('calculates availability states and selectability', () => {
    expect(
      getVariantAvailabilityState({ status: 'active', stock_quantity: 0, alert_threshold: 3 })
    ).toBe('disabled_out_of_stock');
    expect(
      getVariantAvailabilityState({ status: 'active', stock_quantity: 2, alert_threshold: 3 })
    ).toBe('low_stock');
    expect(
      getVariantAvailabilityState({ status: 'active', stock_quantity: 4, alert_threshold: 3 })
    ).toBe('available');
    expect(
      getVariantAvailabilityState({ status: 'archived', stock_quantity: 4, alert_threshold: 3 })
    ).toBe('disabled_archived');
    expect(isVariantSelectable({ status: 'active', stock_quantity: 2, alert_threshold: 3 })).toBe(
      true
    );
    expect(isVariantSelectable({ status: 'active', stock_quantity: 0, alert_threshold: 3 })).toBe(
      false
    );
  });
