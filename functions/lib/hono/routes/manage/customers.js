...
app.post('/batch/tags', zValidator('json', BatchTagsSchema), async (c) => {
  ...
  for (const customer of customers) {
    ...
    statements.push(
      env.DB.prepare('UPDATE customers SET tags = ?, updated_at = ? WHERE id = ?').bind(
...
app.post('/batch/export', zValidator('json', BatchExportSchema), async (c) => {
  ...
  const getChinaDateStr = (ts) => {
    if (!ts) return '';
    return DateUtils.getChinaDateStr(Number(ts));
  };
 ...
 * POST /:id/tags - 添加客户标签
 */
app.post('/:id/tags', zValidator('json', AddTagSchema), async (c) => {
  const { env } = c;
  const id = c.req.param('id');
