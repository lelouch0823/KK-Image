...
const normalizeOptions = (value) => {
    ...
    return Object.fromEntries(Object.entries(options).map(([k, v]) => [String(k || '').toLowerCase(), String(v ?? '')]));
};
...
export const normalizeProductExportFilters = (scope = 'all', filters = {}) => {
    const normalized = Object.fromEntries(PRODUCT_EXPORT_FILTER_KEYS.map((key) => [key, String(filters?.[key] || '').trim()]));
    if (scope !== 'filtered') {
...
const getGroupMerges = (columns, row = 0) => {
    const merges = [];
    let start = 0;
...
export const buildExcelWorkbook = async (rows = [], columns = EXPORT_COLUMNS, meta = {}) => {
    ...
    columns.forEach((_, colIndex) => {
        ...
        if (ws[groupCellAddress])
            ws[groupCellAddress].s = groupHeaderStyle;
        const fieldCellAddress = `${colToExcelName(colIndex)}4`;
        if (ws[fieldCellAddress])
            ws[fieldCellAddress].s = fieldHeaderStyle;
    });
    if (ws.A1)
        ws.A1.s = titleStyle;
    if (ws.A2)
        ws.A2.s = metaStyle;
    const stockFlagIndex = columns.findIndex((col) => col.key === 'stock_flag');
