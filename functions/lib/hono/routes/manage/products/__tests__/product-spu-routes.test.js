...
vi.mock('../../../../../../repositories/ProductRepository.js', () => ({
  ProductRepository: class {
    ...
    search(...args) {
      return mockProductRepo.search(...args);
    }
    updateWithMeta(...args) {
  ...
  beforeEach(() => {
    ...
    mockProductRepo.findBySpuBatch.mockImplementation(async (spus = []) => {
      const entries = await Promise.all(
        (Array.isArray(spus) ? spus : []).map(async (spu) => [
          spu,
          await mockProductRepo.findBySpu(spu),
        ])
      );
    ...
    mockVariantRepo.findByProductIds.mockImplementation(async (productIds = []) => {
      const entries = await Promise.all(
        (Array.isArray(productIds) ? productIds : []).map(async (productId) => [
          productId,
          await mockVariantRepo.findByProductId(productId),
        ])
      );
        ...
        try {
          ...
          successes.push({
            ...
            stats: {
              ...
              archivedCount:
                variants?.archivedCount ??
                variants?.deletedCount ??
                plan?.fallbackStats?.archivedCount ??
                0,
              reactivatedCount:
                variants?.reactivatedCount ?? plan?.fallbackStats?.reactivatedCount ?? 0,
            },
            variants,
          });
        } catch (error) {
    ...
    mockCommandIdempotency.reserveCommand.mockResolvedValue({
      ...
      record: { command_id: 'cmd-product-routes-1' },
    });
    ...
    it('requires complete variant business fields', async () => {
      ...
      const res = await app.request(
        'http://localhost/api/manage/products',
        {
          ...
          body: JSON.stringify({
            name: 'Test Product',
            variants: [{ sku: 'SKU-001', price: 100 }],
          }),
        },
        { DB: {}, executionCtx: { waitUntil: vi.fn() } },
        { waitUntil: vi.fn() }
  ...
  describe('POST /batch', () => {
            ...
            items: [
              {
                ...
                spu: 'SPU-BATCH-TRIM',
                variants: [{ ...validVariants[0], sku: '  SKU-TRIM-001  ' }],
              },
            ],
          }),
        },
    ...
    it('should upsert products and variants when found by spu', async () => {
      ...
      expect(mockProductRepo.updateWithMeta).toHaveBeenCalledWith(
        'existing-id',
        expect.objectContaining({ name: 'Batch Prod 1' })
      );
      expect(mockProductRepo.create).not.toHaveBeenCalled();
    ...
    it('should sync dimensions and normalize variant option keys during batch create', async () => {
      ...
      mockDimensionRepo.listByProduct.mockResolvedValueOnce([]).mockResolvedValueOnce([
        {
          id: 'dim-color',
          name: 'Color',
            ...
            items: [
              {
                ...
                brand: 'New Brand',
                spu: 'SPU-SAFE-1',
                variants: [{ ...validVariants[0], sku: 'SKU-SAFE-1', price: 120 }],
              },
            ],
          }),
        },
    ...
    it('should overwrite fields in replace mode', async () => {
            ...
            items: [
              {
                name: 'New Name',
                spu: 'SPU-REPLACE-1',
                variants: [{ ...validVariants[0], sku: 'SKU-REPLACE-1', price: 120 }],
              },
            ],
          }),
        },
      ...
      expect(mockVariantRepo.syncVariants).toHaveBeenCalledWith('existing-id', [
        expect.objectContaining({
          id: 'var-replace-1',
          price: 120,
        }),
    ...
    it('should rollback newly created product when variant sync fails', async () => {
      ...
      mockVariantRepo.findByProductId.mockResolvedValue([]);
      mockVariantRepo.syncVariants.mockRejectedValue(
    ...
    it('should rollback existing product updates when variant sync fails', async () => {
      ...
      mockVariantRepo.syncVariants.mockResolvedValueOnce({
        createdCount: 0,
        updatedCount: 1,
        archivedCount: 0,
        reactivatedCount: 0,
