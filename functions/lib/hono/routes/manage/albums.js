...
app.put('/:id',
    ...
    async (c) => {
        ...
        try {
            ...
            await env.DB.prepare(`UPDATE albums SET ${updates.join(', ')} WHERE id = ?`).bind(...values).run();

            const updated = await env.DB.prepare('SELECT * FROM albums WHERE id = ?').bind(albumId).first();
    ...
    async (c) => {
        ...
        try {
            ...
            const statements = fileIds.map((fileId, index) =>
                env.DB.prepare('INSERT OR IGNORE INTO album_files (album_id, file_id, sort_order) VALUES (?, ?, ?)')
            ...
            // 更新相册时间
            await env.DB.prepare('UPDATE albums SET updated_at = ? WHERE id = ?').bind(Date.now(), albumId).run();
