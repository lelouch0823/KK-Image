 ...
 */
app.patch('/:id/logistics', async (c) => {
    const { env } = c;
    const user = c.get('user');
    const actor = getAdminActor(user);
    const id = c.req.param('id');
    const body = await c.req.json();
    const { trackingNo = '', carrier = 'express' } = body;
    ...
    await env.DB.prepare(
        'UPDATE orders SET currentData = ?, updatedAt = ? WHERE id = ?'
    ).bind(JSON.stringify(currentData), Date.now(), id).run();
