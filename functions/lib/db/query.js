...
export async function query(db, sql, bindings = [], options = {}) {
  ...
  try {
    ...
    const duration = performance.now() - start;
    console.error(`[D1 Query Error] ${duration.toFixed(2)}ms: ${sql.substring(0, 100)}`, error);
    throw error;
  }
}
...
export async function queryFirst(db, sql, bindings = []) {
  ...
  try {
    ...
    if (duration > SLOW_QUERY_THRESHOLD_MS) {
      console.warn(`[D1 Slow Query] ${duration.toFixed(2)}ms: ${sql.substring(0, 100)}`);
    }
...
export async function execute(db, sql, bindings = []) {
  ...
  try {
    const stmt = db.prepare(sql);
    const result = bindings.length > 0 ? await stmt.bind(...bindings).run() : await stmt.run();

    const duration = performance.now() - start;

    if (duration > SLOW_QUERY_THRESHOLD_MS) {
      console.warn(`[D1 Slow Write] ${duration.toFixed(2)}ms: ${sql.substring(0, 100)}`);
    }
