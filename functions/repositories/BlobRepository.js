...
export class BlobRepository {
  ...
  async findByHash(hash) {
    ...
    return await this.db
      .prepare('SELECT content_hash, size, mime_type, ref_count FROM blobs WHERE content_hash = ?')
      .bind(hash)
      .first();
  }
  ...
  async decrementRefCount(hash) {
    ...
    const selectStmt = this.db
      .prepare('SELECT ref_count FROM blobs WHERE content_hash = ?')
      .bind(hash);
  ...
  async deleteByHashIfUnreferenced(hash) {
    await this.db
      .prepare('DELETE FROM blobs WHERE content_hash = ? AND ref_count <= 0')
      .bind(hash)
      .run();
  }
}
