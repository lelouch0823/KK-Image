...
describe('ProductDimensionRepository', () => {
  ...
  it('createDimension should reject duplicate active dimension names on the same product', async () => {
    ...
    await expect(repo.createDimension('prod-1', { name: '  Color  ' })).rejects.toThrow(
  ...
  it('updateDimension should reject renaming to another active dimension name on the same product', async () => {
    ...
    await expect(repo.updateDimension('prod-1', 'dim-size', { name: ' color ' })).rejects.toThrow(
      'duplicate dimension names are not supported'
    );
  });
  ...
  it('mergeKeepByDimensionRemoval should archive deduped active variants', async () => {
    db.prepare.mockImplementation((sql) => {
      const stmt = createPreparedStatement(sql);
      if (
        sql.includes("SELECT * FROM product_variants WHERE product_id = ? AND status = 'active'")
      ) {
  ...
  it('restoreSnapshot should restore dimensions/values and archive extras', async () => {
    ...
    expect(db.batch).toHaveBeenCalledTimes(1);
    const statements = db.batch.mock.calls[0][0];
    expect(statements.some((stmt) => stmt.sql.includes('INSERT INTO product_dimensions'))).toBe(
      true
    );
    expect(
      statements.some((stmt) => stmt.sql.includes('INSERT INTO product_dimension_values'))
    ).toBe(true);
    expect(
      statements.some((stmt) =>
        stmt.sql.includes("UPDATE product_dimensions SET status = 'archived'")
      )
    ).toBe(true);
  });
  ...
  it('addValue should reject duplicate labels after trim/case normalization', async () => {
    ...
    await expect(repo.addValue('prod-1', 'dim-color', { value: '  RED  ' })).rejects.toThrow(
      'duplicate dimension values with same label are not supported'
    );
  });
  ...
  it('archiveDimension should cascade active values under the archived dimension', async () => {
    ...
    expect(
      statements.filter((stmt) =>
        stmt.sql.includes("UPDATE product_dimension_values SET status = 'archived'")
      )
    ).toHaveLength(2);
  });
  ...
  it('restoreValue should reject duplicate labels after trim/case normalization', async () => {
    ...
    await expect(repo.restoreValue('prod-1', 'val-red-archived')).rejects.toThrow(
      'duplicate dimension values with same label are not supported'
    );
  });
