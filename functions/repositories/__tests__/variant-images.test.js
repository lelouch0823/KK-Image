...
describe('VariantImageRepository', () => {
  ...
  it('promotes the first image to primary even when isPrimary is omitted', async () => {
    ...
    const insertStatement = db.prepare.mock.results.find((entry) =>
      entry.value.sql.includes('INSERT INTO variant_images')
    )?.value;
  ...
  it('rejects duplicate image links for the same variant', async () => {
    ...
    expect(db.prepare).not.toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO variant_images')
    );
  });
    ...
    db.prepare.mockImplementation((sql) => {
      ...
      if (sql.includes('FROM product_variants')) {
        ...
        stmt.run.mockRejectedValue(
          new Error(
            'D1_ERROR: UNIQUE constraint failed: variant_images.variant_id, variant_images.image_id'
          )
        );
      }
      return stmt;
    ...
    db.prepare.mockImplementation((sql) => {
      ...
      if (sql.includes('FROM product_variants')) {
        ...
        stmt.all.mockResolvedValue({
          results: [{ image_id: 'file_1' }, { image_id: 'file_2' }, { image_id: 'file_3' }],
        });
      } else if (sql.includes('MAX(sort_order)')) {
        stmt.first.mockResolvedValue({ max_sort_order: 2 });
      } else if (sql.includes('UPDATE variant_images')) {
  ...
  it('deletes an image link and preserves valid operation scope', async () => {
    db.prepare.mockImplementation((sql) => {
      ...
      if (sql.includes('FROM product_variants')) {
        ...
        sql.includes('SELECT * FROM variant_images WHERE variant_id = ? AND image_id = ?')
      ) {
        stmt.first.mockResolvedValue({
          id: 'vi_2',
          variant_id: 'variant_1',
  ...
  it('promotes the next image to primary when deleting the current primary image', async () => {
    db.prepare.mockImplementation((sql) => {
      ...
      if (sql.includes('FROM product_variants')) {
        ...
        sql.includes('FROM variant_images') &&
        sql.includes('image_id <> ?') &&
        sql.includes('ORDER BY sort_order ASC')
      ) {
        stmt.first.mockResolvedValue({
          image_id: 'file_2',
        });
  ...
  it('normalizes sync image payload: dedupe and enforce single primary', async () => {
    db.prepare.mockImplementation((sql) => {
      ...
      if (sql.includes('FROM product_variants')) {
        ...
        sql.includes('DELETE FROM variant_images') ||
        sql.includes('INSERT INTO variant_images')
      ) {
        stmt.run.mockResolvedValue({ meta: { changes: 1 } });
      }
      return stmt;
