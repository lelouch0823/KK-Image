...
export class ProductRepository {
    ...
    async updateWithMeta(id, updates) {
        ...
        try {
            const result = await execute(this.db, `UPDATE products SET ${built.clause} WHERE id = ?`, built.values, { label: 'product.update' });
            return {
     ...
     */
    async findBySpu(spu) {
        const result = await queryFirst(this.db, this._productSelectSQL('p.spu = ?'), [spu], { label: 'product.findBySpu' });
    ...
    async findBySpuBatch(spus = []) {
        ...
        for (const spuChunk of chunkArray(normalizedSpus, 100)) {
            ...
            const result = await query(this.db, this._productSelectSQL(`p.spu IN (${placeholders})`), spuChunk, { label: 'product.findBySpuBatch' });
    ...
    async bulkUpsertFromImport(plans = []) {
        ...
        if (batchEntries.length > 0) {
            ...
            for (const chunk of chunks) {
                try {
                    await executeBatchChunks(this.db, chunk.map((e) => e.statement));
     ...
     */
    async findById(id) {
        const result = await queryFirst(this.db, this._productSelectSQL('p.id = ?'), [id], { label: 'product.findById' });
        return this._parseResult(result);
    }
    /**
     * 根据条件搜索
    ...
    async search(filters = {}, options = {}) {
        ...
        const { page: safePage, limit: safeLimit, offset } = parseRepoPagination({ page: filters.page, limit: filters.limit }, { defaultPage: 1, defaultLimit: 20, maxLimit: 100 });
     ...
     * @returns 分页的变体列表
     */
    async searchVariants(options = {}) {
        const { page: safePage, limit: safeLimit, offset } = parseRepoPagination({ page: options.page, limit: options.limit }, { defaultPage: 1, defaultLimit: 50, maxLimit: 100 });
        const conditions = ['pv.status = ?'];
        const countParams = ['active'];
        const listParams = ['active'];
    ...
    async suggest(query, limit = 10) {
        ...
        const { results } = await this.db.prepare(`SELECT id, name, brand, spu FROM products
