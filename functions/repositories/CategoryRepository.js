...
export class CategoryRepository {
  ...
  async findById(id) {
    const result = await this.db
      .prepare('SELECT id, name, parent_id, sort_order, created_at FROM categories WHERE id = ?')
      .bind(id)
      .first();
    return result || null;
  }
  ...
  async create(data) {
    await this.db
      ...
      .bind(data.id, data.name, data.parentId || null, data.sortOrder ?? 0, data.createdAt)
      .run();
  ...
  async update(id, updates) {
    ...
    const result = await this.db
      .prepare(`UPDATE categories SET ${clause} WHERE id = ?`)
      .bind(...values, id)
      .run();
  ...
  async getProductIds(categoryId) {
    const { results } = await this.db
      .prepare('SELECT product_id FROM product_categories WHERE category_id = ?')
      .bind(categoryId)
      .all();
  ...
  async setProducts(categoryId, productIds) {
    // 先删除旧关联
    await this.db
      .prepare('DELETE FROM product_categories WHERE category_id = ?')
      .bind(categoryId)
      .run();
  ...
  async getProductCounts() {
    const { results } = await this.db
      .prepare('SELECT category_id, COUNT(*) as count FROM product_categories GROUP BY category_id')
      .all();
