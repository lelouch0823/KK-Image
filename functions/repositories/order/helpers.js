...
function mapFallbackOrderLine(line = {}, order = {}, index = 0, fallbackStatus = 'none') {
  return {
    ...
    orderedQuantity: toNonNegativeNumber(
      line.orderedQuantity ?? line.ordered_qty ?? line.quantity,
      0
    ),
...
function buildFallbackOrderLines(
  order = {},
  currentData = {},
  originalData = {},
  fallbackStatus = 'none'
  ...
  const rawLines =
    Array.isArray(currentData?.lines) && currentData.lines.length > 0
      ? currentData.lines
      : Array.isArray(originalData?.lines) && originalData.lines.length > 0
        ? originalData.lines
        : [];
  ...
  const snapshotName = normalizeSnapshotText(currentData?.name ?? originalData?.name, '');
  ...
  return projectOrderLineStatus(
    lines.reduce(
      (acc, line) => ({
        ...
        cancelled_qty:
          acc.cancelled_qty + Number(line?.cancelled_qty ?? line?.cancelledQuantity ?? 0),
      }),
      {
        ordered_qty: 0,
        procured_qty: 0,
...
function hasListFulfillmentFacts(order) {
  return [order?.line_ordered_qty, order?.line_shipped_qty, order?.line_cancelled_qty].some(
...
export function mapOrderDetail(order) {
  ...
  const lines =
    persistedLines.length > 0
      ? persistedLines
      : buildFallbackOrderLines(
          order,
          currentData,
          originalData,
          order.display_status || procurementStatus
        );
  const displayStatus =
    order.display_status || aggregateOrderDisplayStatus(persistedLines) || procurementStatus;
  const rolledUpLineTotals = persistedLines.reduce(
    (acc, line) => ({
