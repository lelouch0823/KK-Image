...
export class VariantImageRepository {
  ...
  async addImage({ productId, variantId, imageId, isPrimary = false }) {
    ...
    try {
      if (shouldBePrimary) {
        await executeBatchChunks(this.db, [
          this.db
            .prepare(
              'UPDATE variant_images SET is_primary = 0, updated_at = ? WHERE variant_id = ?'
            )
    ...
    return await this.db.prepare('SELECT * FROM variant_images WHERE id = ?').bind(id).first();
  }
  ...
  normalizeIncomingImages(images = []) {
    ...
    for (const item of images) {
      const rawImageId = typeof item === 'string' ? item : (item?.image_id ?? item?.id);
