...
export class SalespersonRepository {
    ...
    async list({ page = 1, limit = 50, search = '' }) {
        const { limit: safeLimit, offset } = parseRepoPagination({ page, limit }, { defaultPage: 1, defaultLimit: 50, maxLimit: 100 });
        let whereClause = '1=1';
        const bindings = [];
        if (search) {
        ...
        while (retries > 0) {
            try {
                await this.db
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
                `)
                    .bind(id, name.trim(), store || null, phone || null, accessToken, passwordHash, timestamp, timestamp)
                    .run();
                break;
            }
            catch (e) {
     ...
     * @returns 密码是否匹配
     */
    async verifyAndMigratePassword(salespersonId, password, encodedHash) {
