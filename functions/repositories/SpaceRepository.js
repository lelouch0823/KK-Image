...
export class SpaceRepository {
    ...
    static ALLOWED_UPDATE_COLUMNS = new Set([
    ...
    async getStats(id, days, startTimestamp) {
      ...
      WHERE sf.space_id = ?`)
        ...
        const { results: trendData } = await this.db
            .prepare(`SELECT DATE(accessed_at / 1000, 'unixepoch', '+8 hours') as date, COUNT(*) as count
    ...
    async create(data) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
            .bind(data.id, data.name, data.description, data.isPublic ? 1 : 0, data.password, data.shareToken, data.expiresAt, data.template, data.templateData, data.shareMode || 'none', data.productId || null, data.variantId || null, data.createdAt, data.updatedAt)
            .run();
        return { id: data.id };
    }
    /**
    ...
    async addFiles(spaceId, fileIds) {
        const statements = fileIds.map((fileId, index) => this.db
            .prepare('INSERT INTO space_files (space_id, file_id, sort_order, added_at) VALUES (?, ?, ?, ?)')
    ...
    async createSubspace(data) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
            .bind(data.id, data.parentId, data.name, data.description, data.isPublic ? 1 : 0, data.password, data.shareToken, data.expiresAt, data.template, data.templateData, data.shareMode || 'none', data.productId || null, data.variantId || null, data.createdAt, data.updatedAt)
            .run();
        return { id: data.id };
    }
    /**
    ...
    async updateSharedSalespersons(id, salespersonIds) {
        ...
        if (salespersonIds.length > 0) {
            const insertStmt = this.db.prepare('INSERT INTO space_salesperson_shares (space_id, salesperson_id, shared_at) VALUES (?, ?, ?)');
