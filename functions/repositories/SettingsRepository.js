...
export class SettingsRepository {
     ...
     */
    async getAllGrouped() {
        const { results } = await this.db.prepare('SELECT * FROM SystemSettings ORDER BY category, "key"').all();
    ...
    async batchUpsert(settings) {
        const stmt = this.db.prepare(`INSERT INTO SystemSettings ("key", "value", "category", "description", "updatedAt")
    ...
    async upsert(key, data) {
        await this.db.prepare(`INSERT INTO SystemSettings ("key", "value", "category", "description", "updatedAt")
       VALUES (?, ?, ?, ?, strftime('%s', 'now'))
       ON CONFLICT("key") DO UPDATE SET
       "value" = excluded."value",
       "updatedAt" = strftime('%s', 'now')`).bind(key, data.value, data.category || 'general', data.description || null).run();
    }
}
