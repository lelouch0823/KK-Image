...
export class ProductDimensionRepository {
  ...
  async findDuplicateValueByLabel(
    dimensionId,
    value,
    { productId = null, excludeValueId = null } = {}
  ) {
    ...
    return this.db
      .prepare(
        'SELECT id FROM product_dimension_values WHERE dimension_id = ? AND LOWER(TRIM(value)) = LOWER(TRIM(?)) LIMIT 1'
      )
  ...
  async findValueRowsByProductAndValueId(productId, valueId) {
    const result = await this.db
      .prepare(
        `SELECT v.id, v.value, v.dimension_id, v.status
                FROM product_dimension_values v
                JOIN product_dimensions d ON d.id = v.dimension_id
                WHERE d.product_id = ?
                  AND v.dimension_id = (
  ...
  async listByProduct(productId) {
    const dimensionsResult = await this.db
      .prepare(
        'SELECT * FROM product_dimensions WHERE product_id = ? ORDER BY sort_order ASC, created_at ASC'
      )
      .bind(productId)
      .all();
    ...
    const valuesResult = await this.db
      .prepare(
        `SELECT * FROM product_dimension_values WHERE dimension_id IN (${placeholders}) ORDER BY sort_order ASC, created_at ASC`
      )
      .bind(...dimensionIds)
      .all();
    const aliasesResult = await this.db
      .prepare(
        `SELECT * FROM product_dimension_aliases WHERE dimension_id IN (${placeholders}) ORDER BY created_at DESC`
      )
      .bind(...dimensionIds)
      .all();
  ...
  async getDimensionMap(productId) {
    const result = await this.db
      .prepare(
        'SELECT id, name FROM product_dimensions WHERE product_id = ? ORDER BY sort_order ASC, created_at ASC'
      )
      .bind(productId)
      .all();
  ...
  /**
   * 批量获取多个商品的 dimension map（避免 N+1 查询）
   * @param {string[]} productIds
   * @returns {Promise<Map<string, Object>>} productId -> { dimensionId: name }
   */
  async getDimensionMapByProductIds(productIds = []) {
    const normalizedIds = [
      ...new Set(
    ...
    for (let i = 0; i < normalizedIds.length; i += chunkSize) {
      ...
      const result = await this.db
        .prepare(
          `SELECT id, name, product_id FROM product_dimensions WHERE product_id IN (${placeholders}) ORDER BY sort_order ASC, created_at ASC`
        )
        .bind(...chunk)
        .all();
  ...
  async createDimension(productId, payload = {}) {
    ...
    const countRow = await this.db
      .prepare(
        "SELECT COUNT(*) AS total FROM product_dimensions WHERE product_id = ? AND status = 'active'"
      )
      .bind(productId)
      .first();
  ...
  async updateDimension(productId, dimensionId, payload = {}) {
    ...
    const nextSortOrder = Number.isInteger(payload.sort_order)
      ? payload.sort_order
      : current.sort_order;
    const timestamp = now();
    await this.db
      .prepare(
        'UPDATE product_dimensions SET name = ?, sort_order = ?, updated_at = ? WHERE id = ?'
      )
    ...
    if (nextName !== current.name) {
      await this.db
        .prepare(
          'INSERT INTO product_dimension_aliases (id, dimension_id, from_name, to_name, created_at) VALUES (?, ?, ?, ?, ?)'
        )
    ...
    return this.db
      .prepare('SELECT * FROM product_dimensions WHERE id = ?')
      .bind(dimensionId)
      .first();
  }
  ...
  async addValue(productId, dimensionId, payload = {}) {
    ...
    const countRow = await this.db
      .prepare(
        "SELECT COUNT(*) AS total FROM product_dimension_values WHERE dimension_id = ? AND status = 'active'"
      )
      .bind(dimensionId)
      .first();
    const sortOrder = Number.isInteger(payload.sort_order)
  ...
  async updateValueMeta(productId, dimensionId, valueId, meta) {
    ...
    const valueRow = await this.db
      ...
      .bind(valueId, productId, dimensionId)
      .first();
    ...
    for (const dimension of snapshot || []) {
      ...
      for (const value of existingDimension?.values || []) {
        ...
        statements.push(
          this.db
            .prepare(
              "UPDATE product_dimension_values SET status = 'archived', updated_at = ? WHERE id = ?"
            )
  ...
  async archiveDimension(productId, dimensionId) {
    ...
    const valueRows = await this.db
      .prepare(
        "SELECT id FROM product_dimension_values WHERE dimension_id = ? AND status = 'active'"
      )
      .bind(dimensionId)
      .all();
    const statements = [
  ...
  async restoreValue(productId, valueId) {
    const row = await this.db
      .prepare(
        `SELECT v.*, d.product_id, d.status AS dimension_status
                FROM product_dimension_values v
                JOIN product_dimensions d ON d.id = v.dimension_id
                WHERE v.id = ? AND d.product_id = ?`
      )
  ...
  async getImpactPreview(productId, payload = {}) {
    ...
    const variantsResult = await this.db
      .prepare(
        "SELECT id, sku, options_values, status FROM product_variants WHERE product_id = ? AND status = 'active' ORDER BY created_at ASC"
      )
      .bind(productId)
      .all();
    ...
    if (action === 'archive_dimension') {
      ...
      affected = variants.filter((item) =>
        Object.prototype.hasOwnProperty.call(item.options_values || {}, dimensionId)
      );
      ...
      affected = variants.filter(
        (item) => (item.options_values || {})[valueRow.dimension_id] === valueRow.value
      );
    } else {
  ...
  async archiveVariantsByValue(productId, valueId) {
    ...
    return {
      ...
      dimensionId: valueRow.dimension_id,
  ...
  async mergeKeepByDimensionRemoval(productId, dimensionId) {
    const list = await this.db
      .prepare(
        "SELECT * FROM product_variants WHERE product_id = ? AND status = 'active' ORDER BY created_at ASC"
      )
      .bind(productId)
      .all();
    ...
    for (const row of rows) {
      ...
      if (seen.has(signature)) {
        ...
        statements.push(
          this.db
            .prepare("UPDATE product_variants SET status = 'archived', updated_at = ? WHERE id = ?")
      ...
      statements.push(
        this.db
          .prepare(
            'UPDATE product_variants SET options_values = ?, variant_signature = ?, updated_at = ? WHERE id = ?'
          )
