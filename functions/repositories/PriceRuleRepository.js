...
export class PriceRuleRepository {
    ...
    async findByVariantIds(variantIds = []) {
        ...
        for (let i = 0; i < ids.length; i += chunkSize) {
            ...
            const result = await this.db
                .prepare(`SELECT * FROM price_rules WHERE variant_id IN (${placeholders}) ORDER BY variant_id, price_type ASC`)
                .bind(...chunk)
                .all();
    ...
    async upsert(variantId, priceType, price, validFrom = null, validTo = null) {
        ...
        await this.db
            ...
            .run();
        return { id, variant_id: variantId, price_type: priceType, price: Number(price), valid_from: validFrom, valid_to: validTo, updated_at: timestamp };
    }
    ...
    async delete(id) {
        const result = await this.db
            .prepare('DELETE FROM price_rules WHERE id = ?')
            .bind(id)
            .run();
