...
function detectStructuredToolPattern(text) {
    return /"name"\s*:\s*"[^"]+"\s*,\s*"arguments"\s*:\s*[{[]/i.test(text)
        || /<tools\b/i.test(text)
        || /<arg_key>/i.test(text)
...
export class ContentGate {
    constructor(options = {}) {
        ...
        this.toolNames = Array.isArray(options.toolNames) && options.toolNames.length > 0
            ? options.toolNames
            : DEFAULT_TOOL_NAMES;
        this.dangerTags = Array.isArray(options.dangerTags) && options.dangerTags.length > 0
            ? options.dangerTags
            : DEFAULT_DANGER_TAGS;
        this.stats = {
            inputChars: 0,
    ...
    _tryRecover() {
        ...
        const splitAbsolute = this.blockedContent.length - tail.length + splitMatch.index + splitMatch[0].length;
        const candidate = this.blockedContent.slice(splitAbsolute);
        if (!candidate) return '';

        if (this._findStrongIndex(candidate) >= 0 || findWeakToolNameIndex(candidate, this.toolNames) >= 0) {
            return '';
        }
