...
export class AIService {
  ...
  async _writeTelemetry({
    ...
    await telemetryWriter.writeAll({
      trace: createAITraceRecord({
        ...
        userId: user?.id || null,
        routeType,
        quotaDecision: quotaDecision?.reason || 'allowed',
        ...extra,
      }),
  ...
  async handleChat({ body, runtimeEnv, user, c, quotaDecision }) {
    const ctx = await prepareAIRequest({
      body,
      runtimeEnv,
      user,
      c,
    ...
    requestContext.addSpan(
      createAISpanRecord({
        spanType: 'request_start',
        status: 'started',
        detail: { routeType: 'chat' },
      })
    );
    ...
    if (actionHandle.handled) {
      ...
      return {
        ...
        message: {
          role: 'assistant',
          content:
            actionHandle.actionResult.payload?.successMessage || actionHandle.actionResult.kind,
    ...
    logModelUsageTelemetry('Chat', {
      ...
      visionFirst,
      toolsEnabled: !visionFirst && tools.length > 0,
      phase: 'initial',
    });
  ...
  async handleStream({ body, runtimeEnv, user, c, quotaDecision, emit }) {
    ...
    if (actionHandle.handled) {
      ...
      if (actionHandle.refreshEvent) {
        await emit({
          type: actionHandle.refreshEvent.type,
          data: actionHandle.refreshEvent.data || {},
        });
      }
      ...
      console.info(
        ...
        JSON.stringify({
          ...
          suspectedFalsePositive:
            gateStats.blockedEvents > 0 &&
            toolCalls.length === 0 &&
            roundTelemetry.executedTools === 0,
        })
      );
    }
