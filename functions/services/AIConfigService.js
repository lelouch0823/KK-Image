  ...
  async resolveRuntimeEnv() {
    try {
      ...
      return {
        ...
        AI_DYNAMIC_FALLBACK_ENABLED: String(dynamicFallback !== undefined ? dynamicFallback : this.env.AI_DYNAMIC_FALLBACK_ENABLED || 'false'),
        AI_MODEL_HEALTH_WINDOW: String(healthWindow !== undefined ? healthWindow : this.env.AI_MODEL_HEALTH_WINDOW || '20'),
        AI_MODEL_SWITCH_THRESHOLD: String(switchThreshold !== undefined ? switchThreshold : this.env.AI_MODEL_SWITCH_THRESHOLD || '5'),
        AI_STREAM_GATE_ENABLED: String(streamGateEnabled !== undefined ? streamGateEnabled : this.env.AI_STREAM_GATE_ENABLED || 'true'),
        AI_STREAM_GATE_STRICT_MODE: String(streamGateStrictMode !== undefined ? streamGateStrictMode : this.env.AI_STREAM_GATE_STRICT_MODE || 'false'),
        AI_MAX_TOOL_ROUNDS: maxToolRounds !== undefined ? maxToolRounds : (this.env.AI_MAX_TOOL_ROUNDS ? parseInt(this.env.AI_MAX_TOOL_ROUNDS, 10) : 3),
        AI_MAX_TOOLS_PER_ROUND: maxToolsPerRound !== undefined ? maxToolsPerRound : (this.env.AI_MAX_TOOLS_PER_ROUND ? parseInt(this.env.AI_MAX_TOOLS_PER_ROUND, 10) : 8),
      };
    } catch (error) {
      ...
      return {
        ...serializableEnv,
        AI_MAX_TOOL_ROUNDS: this.env.AI_MAX_TOOL_ROUNDS ? parseInt(this.env.AI_MAX_TOOL_ROUNDS, 10) : 3,
        AI_MAX_TOOLS_PER_ROUND: this.env.AI_MAX_TOOLS_PER_ROUND ? parseInt(this.env.AI_MAX_TOOLS_PER_ROUND, 10) : 8,
      };
    }
  }
}
