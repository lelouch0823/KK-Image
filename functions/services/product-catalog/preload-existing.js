...
export async function preloadBatchImportExistingState({
  ...
  const spuList = [
    ...new Set((items || []).map((item) => normalizeSpu(item?.spu)).filter(Boolean)),
  ];
  ...
  if (spuList.length > 0) {
    if (typeof productRepo?.findBySpuBatch === 'function') {
      ...
      const entries = await Promise.all(
        spuList.map(async (spu) => [spu, await productRepo.findBySpu(spu)])
      );
  ...
  const productIds = [
    ...new Set(existingProducts.map((product) => String(product?.id || '').trim()).filter(Boolean)),
  ];
