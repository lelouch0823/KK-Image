...
import { buildSafeVariantSyncPayload, mergeIncomingWithExisting } from './variant-matching.js';

const formatImportItemName = (plan) =>
  String(plan?.normalizedItem?.spu || plan?.normalizedItem?.name || 'UNKNOWN');
...
async function rollbackFailedImportPlan({ db, plan, productRepo, variantRepo, dimensionRepo }) {
  if (!plan) return;
  ...
  if (plan.existingVariantsSnapshot && typeof variantRepo?.syncVariants === 'function') {
    await variantRepo.syncVariants(
      plan.productId,
      buildCatalogRollbackPayload(plan.existingVariantsSnapshot)
    );
  }
}

async function buildVariantSyncPlan({ plan, importMode, conflicts, syncDimensionsFromPayload }) {
  ...
  const existingVariants = plan.operation === 'created' ? [] : plan.existingVariantsSnapshot || [];
  ...
  const incomingVariantCount = Array.isArray(normalizedItem.variants)
    ? normalizedItem.variants.length
    : 0;
  const matchedUpdateCount = nextVariantsToSync.reduce(
    (count, variant) => (variant?.id && existingIdSet.has(variant.id) ? count + 1 : count),
    0
  );
...
export async function executeBulkProductImportUpsert({
  ...
  const productResult = await productRepo.bulkUpsertFromImport(
    plans.map((plan) => ({
      itemKey: plan.itemKey,
  ...
  for (const failure of productResult?.failures || []) {
    ...
    errors.push(
      `Failed to process item ${itemLabel}: ${failure.error?.message || 'Product upsert failed'}`
    );
  }
  ...
  const successfulVariantMap = new Map(
    (variantResult?.successes || []).map((entry) => [entry.itemKey, entry])
  );
  const failedVariantMap = new Map(
    (variantResult?.failures || []).map((entry) => [entry.itemKey, entry])
  );
  ...
  for (const plan of productSuccessPlans) {
    ...
    if (failedVariant) {
      ...
      const itemLabel = formatImportItemName(plan);
      errors.push(
        `Failed to process item ${itemLabel}: ${failedVariant.error?.message || 'Variant upsert failed'}`
      );
      continue;
    }
